    ...
    vm_mock_admin_text *page, const vm_net_mock_chest_admin_row *chest)
{
    ...
    vm_mock_admin_text_appendf(
        ...
        "<span>当前概率</span><span>操作</span></div>",
                                            ...
                                            const char *query)
{
    ...
    for (u32 i = 0; i < chestCount; ++i)
    {
        vm_mock_admin_text_appendf(
            &page,
            "<button type=\"button\" class=\"chest-select%s\" "
            "data-chest-select=\"%u\" aria-pressed=\"%s\"><span>%s</span>"
            "<small>%u 项</small></button>",
            chests[i].chestItemId == selectedChest ? " on" : "",
            chests[i].chestItemId,
            chests[i].chestItemId == selectedChest ? "true" : "false",
            g_vm_net_mock_chest_kinds[i].name, chests[i].rewardCount);
    }
    vm_mock_admin_text_appendf(
        &page,
        "</div></aside><section class=\"chest-editor\" data-chest-editor-root "
        "data-chest-selected=\"%u\">",
        selectedChest);
    for (u32 i = 0; i < chestCount; ++i)
    {
        ...
        vm_mock_admin_text_appendf(
            &page,
            "<article id=\"chest-%u\" class=\"card chest-card\" data-chest-editor=\"%u\"%s><div class=\"card-head\"><div><h2>%s <span class=\"badge\">ID %u</span></h2><p class=\"hint\">开启条件：消耗 1 个 %s（ID %u）。%s</p></div><span class=\"badge\">已配置 %u 项</span></div><form method=\"post\" action=\"/action\"><input type=\"hidden\" name=\"action\" value=\"save-chest-rewards\"><input type=\"hidden\" name=\"chest_item_id\" value=\"%u\"><input type=\"hidden\" name=\"key_item_id\" value=\"%u\">",
            chests[i].chestItemId, chests[i].chestItemId,
            chests[i].chestItemId == selectedChest ? "" : " hidden",
        ...
        vm_mock_admin_text_appendf(
            &page,
            "<p class=\"hint\">每行需同时填写物品、数量和权重；三项都为 0 的行会忽略。最多 %u 项；装备、神仙壶和逍遥壶的数量必须为 1；同一物品不能重复出现。</p><div class=\"actions chest-reward-actions\"><button type=\"button\" data-chest-reward-add>＋ 添加奖励</button></div><div class=\"actions\"><button type=\"submit\">保存奖池</button></div></form>",
            VM_NET_MOCK_CHEST_REWARD_MAX);
