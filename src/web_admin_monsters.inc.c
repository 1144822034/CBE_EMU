                                               ...
                                               const char *query)
{
    ...
    vm_mock_admin_text_appendf(
        &page,
        "<form class=\"actions\" method=\"post\" action=\"/action\" data-monster-action data-confirm-message=\"仅按当前等级和怪物类型重新计算 HP、MP、攻击、防御；经验、金钱、掉落与其他配置保持不变。是否继续？\"><input type=\"hidden\" name=\"action\" value=\"reset-monster-combat-stats\"><input type=\"hidden\" name=\"monster_id\" value=\"%u\"><button class=\"secondary\" type=\"submit\">重置战斗四项属性</button></form>",
        edit->enemyId);
    if (edit->overridden)
    {
        vm_mock_admin_text_appendf(
