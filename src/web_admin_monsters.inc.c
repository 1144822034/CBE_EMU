                                               ...
                                               const char *query)
{
    ...
    vm_mock_admin_text_appendf(
        ...
        "<div class=\"grid\"><aside class=\"card catalog\" data-monster-batch-manager><input class=\"search\" id=\"monster-search\" placeholder=\"按 ID、名称或场景筛选\"><strong>怪物目录（%u）</strong><a class=\"monster-export\" href=\"/monster-boss-drops.xlsx\" download>导出首领掉落 Excel</a><form class=\"monster-auto-drop\" method=\"post\" action=\"/action\" data-monster-action data-confirm-message=\"将覆盖所有怪物的非任务掉落：品质 0 只分配给普通怪，品质 1 和 2 只分配给首领。同一部位同一品质只保留一个掉落，怪物自带任务材料会保留。是否继续？\"><input type=\"hidden\" name=\"action\" value=\"assign-monster-equipment-drops\"><input type=\"hidden\" name=\"monster_id\" value=\"%u\"><strong>智能分配装备掉落</strong><div class=\"monster-auto-drop-fields\"><label>品质 0（%%）<input name=\"quality0_drop_rate\" type=\"number\" min=\"0.01\" max=\"100\" step=\"0.01\" value=\"5\" required></label><label>品质 1（%%）<input name=\"quality1_drop_rate\" type=\"number\" min=\"0.01\" max=\"100\" step=\"0.01\" value=\"2\" required></label><label>品质 2（%%）<input name=\"quality2_drop_rate\" type=\"number\" min=\"0.01\" max=\"100\" step=\"0.01\" value=\"1\" required></label></div><button type=\"submit\">智能分配并覆盖</button><p class=\"hint\">覆盖主武器及全部穿戴部位的品质 0、1、2 装备；支持最多两位小数概率，同品质尽量均分并贴近怪物等级，名称连续匹配至少两个中文字符时优先。</p></form><div class=\"monster-batch-tools\"><button class=\"secondary\" type=\"button\" data-monster-batch-select-visible>全选当前筛选</button><button class=\"secondary\" type=\"button\" data-monster-batch-clear>清除选择</button><form method=\"post\" action=\"/action\" data-monster-action data-confirm-message=\"将按各自当前等级与类型重新计算所选怪物的 HP、MP、攻击、防御；经验、铜钱与物品掉落不变。是否继续？\"><input type=\"hidden\" name=\"action\" value=\"reset-monster-combat-stats-bulk\"><input type=\"hidden\" name=\"monster_id\" value=\"%u\"><input type=\"hidden\" name=\"monster_ids\" value=\"\" data-monster-batch-ids><button class=\"danger\" type=\"submit\" data-monster-batch-reset disabled>批量重置四项属性（0）</button></form></div><div class=\"list\" id=\"monster-list\" data-admin-list>",
        monsterCount, selectedMonsterId, selectedMonsterId);
