    ...
    vm_mock_admin_text *page, const vm_net_mock_monster_admin_row *monster)
{
    ...
    vm_mock_admin_text_appendf(page, "<div class=\"drop-list\" id=\"monster-drop-list\">");
    for (u8 slot = 0; slot < VM_NET_MOCK_MONSTER_DROP_MAX; ++slot)
    {
        ...
        vm_mock_admin_text_appendf(
            page, "<div class=\"drop-row\" data-drop-row%s><span>掉落 #%u</span>",
            slot < visibleRows ? "" : " hidden", (u32)slot + 1u);
    ...
    vm_mock_admin_text_appendf(
        page,
        "</div><button class=\"drop-add\" id=\"monster-drop-add\" type=\"button\">＋ 新增掉落</button>"
        "<p class=\"hint\">每条掉落独立按概率投掷；同一物品不能重复配置。留空行会被忽略，移除后保存即删除该掉落。</p>");
}
