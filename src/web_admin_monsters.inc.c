    ...
    vm_mock_admin_xlsx_bytes *sheet, u32 row, u32 style, u32 rate)
{
    ...
    vm_net_mock_format_drop_rate_basis_points((u16)rate, rateText,
                                              sizeof(rateText));
    return vm_mock_admin_xlsx_bytes_appendf(
        sheet, "<c r=\"E%u\" s=\"%u\" t=\"inlineStr\"><is><t>%s%%</t></is></c>",
        row, style, rateText);
}
            ...
            for (u32 dropIndex = 0; dropIndex < monster->dropCount; ++dropIndex)
            {
                ...
                if (!vm_mock_admin_xlsx_bytes_appendf(
                    ...
                    !vm_mock_admin_xlsx_sheet_append_probability_cell(
                        &parts[5], sheetRow, style,
                        drop->rateBasisPoints) ||
                    !vm_mock_admin_xlsx_bytes_append_string(&parts[5], "</row>"))
                {
                    vm_mock_admin_xlsx_bytes_free(&merges);
                    goto done;
    ...
    vm_mock_admin_text *page, const vm_net_mock_monster_admin_row *monster)
{
    ...
    for (u8 slot = 0; slot < VM_NET_MOCK_MONSTER_DROP_MAX; ++slot)
    {
        ...
        u16 rateBasisPoints =
            slot < monster->dropCount
                ? monster->drops[slot].rateBasisPoints
                : 0;
        char rateText[16];
        const vm_net_mock_shop_catalog_item *item =
        ...
        vm_net_mock_format_drop_rate_basis_points(
            rateBasisPoints, rateText, sizeof(rateText));
        ...
        vm_mock_admin_text_appendf(
            page,
            "<label class=\"field\"><span>概率（%%）</span><input data-drop-rate type=\"number\" name=\"drop_rate_%u\" min=\"0\" max=\"100\" step=\"0.01\" value=\"%s\" required></label>"
...
/* Keep the global mutation controls separate from the selected-monster form.
 * The form only supplies an anchor for returning the status message; the
 * server owns the scope filters, catalog validation and atomic persistence. */
static void vm_mock_admin_render_monster_drop_batch_modal(
    vm_mock_admin_text *page, u32 selectedMonsterId)
{
    if (page == NULL)
        return;
    vm_mock_admin_text_appendf(
        page,
        "<div class=\"item-modal monster-drop-batch-modal\" id=\"monster-drop-batch-modal\" role=\"dialog\" aria-modal=\"true\" aria-label=\"怪物掉落批量管理\" hidden><section class=\"item-picker-panel monster-drop-batch-panel\"><div class=\"item-picker-head\"><div><h3>怪物掉落批量管理</h3><p>智能分配与指定物品批量调整均在此配置；提交后立即生效。</p></div><button class=\"item-picker-close\" type=\"button\" data-monster-drop-batch-close aria-label=\"关闭怪物掉落批量管理\">×</button></div><div class=\"monster-drop-batch-sections\"><form class=\"monster-auto-drop\" method=\"post\" action=\"/action\" data-monster-action data-monster-drop-batch-form data-confirm-message=\"将覆盖所有非场景战斗怪的非任务掉落：品质 0 只分配给普通怪，品质 1 和 2 只分配给首领。同一部位同一品质只保留一个掉落，怪物自带任务材料会保留。是否继续？\"><input type=\"hidden\" name=\"action\" value=\"assign-monster-equipment-drops\"><input type=\"hidden\" name=\"monster_id\" value=\"%u\"><div><strong>智能分配装备掉落</strong><p class=\"hint\">覆盖主武器及全部穿戴部位的品质 0、1、2 装备；同品质尽量均分并贴近怪物等级，名称连续匹配至少两个中文字符时优先。</p></div><div class=\"monster-auto-drop-fields\"><label>品质 0（%%）<input name=\"quality0_drop_rate\" type=\"number\" min=\"0.01\" max=\"100\" step=\"0.01\" value=\"5\" required></label><label>品质 1（%%）<input name=\"quality1_drop_rate\" type=\"number\" min=\"0.01\" max=\"100\" step=\"0.01\" value=\"2\" required></label><label>品质 2（%%）<input name=\"quality2_drop_rate\" type=\"number\" min=\"0.01\" max=\"100\" step=\"0.01\" value=\"1\" required></label></div><button type=\"submit\">智能分配并覆盖</button></form><form class=\"monster-drop-bulk-config\" method=\"post\" action=\"/action\" data-monster-action data-monster-drop-batch-form data-confirm-message=\"将按筛选范围批量修改怪物掉落；已配置的同物品不会在增加时重复，场景战斗怪会被保护并跳过。是否继续？\"><input type=\"hidden\" name=\"action\" value=\"batch-configure-monster-drops\"><input type=\"hidden\" name=\"monster_id\" value=\"%u\"><input type=\"hidden\" name=\"drop_batch_item_ids\" value=\"\" data-monster-drop-batch-ids><div><strong>批量增加或删除指定掉落</strong><p class=\"hint\">增加不会覆盖已有掉落；槽位不足的条目会跳过。删除只移除所选物品。场景战斗怪拥有独立配置，始终不受本操作影响。</p></div><div class=\"monster-drop-batch-fields\"><label class=\"field\"><span>操作</span><select name=\"drop_batch_mode\" data-monster-drop-batch-mode><option value=\"add\">批量增加掉落</option><option value=\"remove\">批量删除掉落</option></select></label><label class=\"field\"><span>怪物范围</span><select name=\"drop_batch_family\" data-monster-drop-batch-family><option value=\"all\">全部类型</option><option value=\"boss\">仅首领类</option><option value=\"nonboss\">仅非首领类</option></select></label><label class=\"field\"><span>最低等级</span><input name=\"drop_batch_level_min\" type=\"number\" min=\"1\" max=\"255\" value=\"1\" required></label><label class=\"field\"><span>最高等级</span><input name=\"drop_batch_level_max\" type=\"number\" min=\"1\" max=\"255\" value=\"255\" required></label><label class=\"field\" data-monster-drop-batch-rate-field><span>新增概率（%%）</span><input name=\"drop_batch_rate\" type=\"number\" min=\"0.01\" max=\"100\" step=\"0.01\" value=\"100\" required></label></div><div class=\"monster-drop-batch-picker-tools\"><label><span>物品分类</span><select data-monster-drop-batch-category>",
        selectedMonsterId, selectedMonsterId);
    vm_mock_admin_render_catalog_category_options(page, "全部物品分类");
    vm_mock_admin_text_appendf(
        page,
        "</select></label><label data-monster-drop-batch-quality-field><span>装备品质</span><select data-monster-drop-batch-quality>");
    vm_mock_admin_render_catalog_quality_options(page, "全部品质");
    vm_mock_admin_text_appendf(
        page,
        "</select></label><label><span>搜索物品</span><input type=\"search\" data-monster-drop-batch-search placeholder=\"按名称或 ID 搜索\"></label></div><div class=\"item-result-bar\"><span data-monster-drop-batch-result-count>正在载入物品目录…</span><span data-monster-drop-batch-selection-count>已选择 0 / 12 件</span></div><p class=\"item-picker-error\" data-monster-drop-batch-error></p><div class=\"npc-stock-picker-actions\"><button class=\"secondary\" type=\"button\" data-monster-drop-batch-select-filtered>选择当前筛选</button><button class=\"secondary\" type=\"button\" data-monster-drop-batch-clear>清除选择</button></div><div class=\"item-picker-list monster-drop-batch-picker-list\" data-monster-drop-batch-list></div><p class=\"item-picker-empty\" data-monster-drop-batch-empty hidden>没有符合当前筛选的物品。</p><div class=\"actions monster-drop-batch-actions\"><button class=\"secondary\" type=\"button\" data-monster-drop-batch-close>取消</button><button type=\"submit\" data-monster-drop-batch-submit disabled>批量增加掉落（0）</button></div></form></div></section></div>");
}
                                               ...
                                               const char *query)
{
    ...
    vm_mock_admin_text_appendf(
        ...
        "<div class=\"grid\"><aside class=\"card catalog\" data-monster-batch-manager><input class=\"search\" id=\"monster-search\" placeholder=\"按 ID、名称或场景筛选\"><strong>怪物目录（%u）</strong><a class=\"monster-export\" href=\"/monster-boss-drops.xlsx\" download>导出首领掉落 Excel</a><div class=\"monster-drop-batch-launcher\"><strong>怪物掉落管理</strong><p class=\"hint\">智能装备分配、按首领类型和等级范围批量增删掉落物品。</p><button type=\"button\" data-monster-drop-batch-open aria-haspopup=\"dialog\">打开掉落批量管理</button></div><div class=\"monster-batch-tools\"><button class=\"secondary\" type=\"button\" data-monster-batch-select-visible>全选当前筛选</button><button class=\"secondary\" type=\"button\" data-monster-batch-clear>清除选择</button><form method=\"post\" action=\"/action\" data-monster-action data-confirm-message=\"将按各自当前等级与类型重新计算所选怪物的 HP、MP、攻击、防御、经验、铜钱；物品掉落不变。是否继续？\"><input type=\"hidden\" name=\"action\" value=\"reset-monster-combat-stats-bulk\"><input type=\"hidden\" name=\"monster_id\" value=\"%u\"><input type=\"hidden\" name=\"monster_ids\" value=\"\" data-monster-batch-ids><button class=\"danger\" type=\"submit\" data-monster-batch-reset disabled>批量重置属性与奖励（0）</button></form></div><div class=\"list\" id=\"monster-list\" data-admin-list>",
    ...
    vm_mock_admin_text_appendf(
        &page,
        "</div><p class=\"hint\">保存后会更新服务端战斗计算和结算。怪物名称来自真实 SCE，只读；调整等级或类型不会擅自覆盖手工填写的战斗数值。场景战斗怪草稿还须在“场景战斗怪”中部署，并由客户端重新进入场景后才会创建节点。首领为单体高强度战斗，其血量、攻击和防御按同等级品质 0 装备参照计算；任何玩家都可挑战，但同级单人不应能以常规战斗完成击杀。</p><div class=\"actions\"><button type=\"submit\">保存怪物属性</button></div></form>");
    vm_mock_admin_render_item_picker_modal(&page, false);
    vm_mock_admin_render_monster_drop_picker_modal(&page);
    vm_mock_admin_text_appendf(
        &page,
        "<form class=\"actions\" method=\"post\" action=\"/action\" data-monster-action data-confirm-message=\"按当前等级和怪物类型重新计算 HP、MP、攻击、防御、经验、铜钱；物品掉落与其他配置保持不变。是否继续？\"><input type=\"hidden\" name=\"action\" value=\"reset-monster-combat-stats\"><input type=\"hidden\" name=\"monster_id\" value=\"%u\"><button class=\"secondary\" type=\"submit\">重置属性与奖励</button></form>",
        edit->enemyId);
    if (edit->overridden)
    {
        vm_mock_admin_text_appendf(
