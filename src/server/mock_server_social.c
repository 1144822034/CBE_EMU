...
static bool vm_net_mock_is_current_scene_completion_request(const u8 *request, u32 requestLen)
{
    ...
    bool directTeleportCompletion = false;
     ...
     * destination scene shell, so this is its current-scene completion
     * contract rather than a fresh generic scene-change request.  Keeping it
     * in the generic path defers the no-posinfo WT30/2 until a later WT6/1 and
     * leaves that shell on the loading overlay.
     *
     * Keep the exception deliberately narrow: it requires the live direct
     * teleport provenance, the exact pending target, and the scene currently
     * selected by the preceding 30/1.  It is intentionally not keyed to one
     * map: the direct map-stone contract is shared by destinations such as
     * c00 Punglai and c04 Lin'an.
     */
