...
static const char g_vm_mock_admin_script[] =
    ...
    "const setupEnhanceRestoreTab=()=>{const nav=document.querySelector('#admin-spa-tabs')||document.querySelector('nav.tabs');if(!nav||nav.querySelector('[data-admin-tab=\\\"enhance-restore\\\"],a[href*=\\\"tab=enhance-restore\\\"]'))return;const accounts=nav.querySelector('[data-admin-tab=accounts],a[href*=\\\"tab=accounts\\\"]'),link=document.createElement('a');link.className='tab admin-spa-tab';link.dataset.adminTab='enhance-restore';link.href='?tab=enhance-restore';link.textContent='强化恢复';if(new URL(location.href).searchParams.get('tab')==='enhance-restore'){link.classList.add('on');link.setAttribute('aria-current','page');}accounts?.after(link);};"
