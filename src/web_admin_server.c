...
#define VM_MOCK_ADMIN_ONLINE_PLAYER_STATS_PATH \
    VM_MOCK_ADMIN_BASE_PATH "/online-players"
#define VM_MOCK_ADMIN_MONSTER_BOSS_EXPORT_PATH \
    VM_MOCK_ADMIN_BASE_PATH "/monster-boss-drops.xlsx"
#define VM_MOCK_ADMIN_ROLE_OPERATION_MODAL_Z_INDEX 1100
#define VM_MOCK_ADMIN_ITEM_PICKER_MODAL_Z_INDEX 1200
#define VM_MOCK_ADMIN_STRINGIFY_INNER(value) #value
#define VM_MOCK_ADMIN_STRINGIFY(value) VM_MOCK_ADMIN_STRINGIFY_INNER(value)

/* The main management page renders the live count before the implementation
 * below; keep this declaration so direct server-only regression translation
 * units retain the same strict C contract as the normal service build. */
static u32 vm_mock_admin_online_player_count(void);
...
static int vm_mock_admin_send_response(vm_mock_service_socket client,
                                       ...
                                       const char *body)
{
    /* Redirects can carry URL-encoded Chinese status messages.  The caller's
     * extra-header buffer is 4096 bytes, so the final HTTP header must be
     * larger than that as well; otherwise snprintf would silently discard the
     * tail of Location even after the redirect builder succeeds. */
    char header[8192];
...
static const char g_vm_mock_admin_script[] =
    ...
    "const roleOperationOpen=()=>!!document.querySelector('[data-role-operation-modal]:not([hidden])');"
    ...
    "const setupMonsterDropBatchModal=()=>{const state=window.__cbeMonsterDropBatchState||(window.__cbeMonsterDropBatchState={bound:false,selected:new Set(),choices:[],renderedSource:null}),trigger=document.querySelector('[data-monster-drop-batch-open]'),modal=document.querySelector('#monster-drop-batch-modal'),source=document.querySelector('#item-picker-options');if(!trigger||!modal)return;state.trigger=trigger;state.modal=modal;state.source=source;state.ids=modal.querySelector('[data-monster-drop-batch-ids]');state.mode=modal.querySelector('[data-monster-drop-batch-mode]');state.category=modal.querySelector('[data-monster-drop-batch-category]');state.quality=modal.querySelector('[data-monster-drop-batch-quality]');state.qualityField=modal.querySelector('[data-monster-drop-batch-quality-field]');state.rateField=modal.querySelector('[data-monster-drop-batch-rate-field]');state.search=modal.querySelector('[data-monster-drop-batch-search]');state.list=modal.querySelector('[data-monster-drop-batch-list]');state.result=modal.querySelector('[data-monster-drop-batch-result-count]');state.selection=modal.querySelector('[data-monster-drop-batch-selection-count]');state.error=modal.querySelector('[data-monster-drop-batch-error]');state.empty=modal.querySelector('[data-monster-drop-batch-empty]');state.submit=modal.querySelector('[data-monster-drop-batch-submit]');state.rebuild=()=>{if(state.renderedSource===state.source||!state.list)return;state.list.replaceChildren();state.choices=[];state.options=state.source?[...state.source.options].filter(option=>option.value&&option.value!=='0'):[];state.optionById=new Map(state.options.map(option=>[option.value,option]));state.selected=new Set([...state.selected].filter(value=>state.optionById.has(value)));for(const option of state.options){const choice=document.createElement('label'),check=document.createElement('input'),body=document.createElement('span'),title=document.createElement('strong'),meta=document.createElement('small'),isEquipment=option.dataset.equip==='1';choice.className='item-choice monster-drop-choice';choice.dataset.itemId=option.value;choice.dataset.category=option.dataset.category||'';choice.dataset.quality=option.dataset.quality||'0';choice.dataset.search=option.textContent.toLowerCase();check.type='checkbox';title.textContent=option.textContent;meta.textContent=isEquipment?`品质 ${choice.dataset.quality} · 需求等级 ${option.dataset.level||'0'}`:'普通物品';body.append(title,meta);choice.append(check,body);check.addEventListener('change',()=>{if(check.checked&&state.selected.size>=12){check.checked=false;state.error.textContent='一次最多选择 12 件物品，请先取消其他选择';return;}if(check.checked)state.selected.add(option.value);else state.selected.delete(option.value);state.error.textContent='';state.apply();});state.choices.push(choice);state.list.append(choice);}state.renderedSource=state.source;};state.sync=()=>{const count=state.selected.size,mode=state.mode?.value||'add';if(state.ids)state.ids.value=[...state.selected].join(',');if(state.selection)state.selection.textContent=`已选择 ${count} / 12 件`;if(state.submit){state.submit.disabled=count===0;state.submit.textContent=`${mode==='remove'?'批量删除掉落':'批量增加掉落'}（${count}）`;}if(state.rateField)state.rateField.hidden=mode==='remove';};state.apply=()=>{state.rebuild();const wanted=state.category?.value||'all',keyword=state.search?.value.trim().toLowerCase()||'',supports=wanted==='all'||wanted.startsWith('e');if(state.qualityField)state.qualityField.hidden=!supports;if(!supports&&state.quality)state.quality.value='all';const wantedQuality=state.quality?.value||'all';let shown=0;for(const choice of state.choices){const equipment=choice.dataset.category.startsWith('e'),visible=(wanted==='all'||choice.dataset.category===wanted)&&(!supports||wantedQuality==='all'||(equipment&&choice.dataset.quality===wantedQuality))&&(!keyword||choice.dataset.search.includes(keyword));choice.hidden=!visible;const check=choice.querySelector('input');check.checked=state.selected.has(choice.dataset.itemId);choice.classList.toggle('selected',check.checked);if(visible)shown++;}if(state.result)state.result.textContent=`找到 ${shown} 件可批量配置物品`;if(state.empty)state.empty.hidden=shown!==0;state.sync();};state.hide=()=>{if(!state.modal)return;state.modal.hidden=true;document.body.classList.remove('modal-open');state.trigger?.focus();};state.show=()=>{state.modal.hidden=false;document.body.classList.add('modal-open');state.error.textContent='';state.apply();state.search?.focus();};if(!state.bound){state.bound=true;document.addEventListener('click',event=>{const open=event.target.closest('[data-monster-drop-batch-open]');if(open){event.preventDefault();state.show();return;}if(event.target.closest('[data-monster-drop-batch-close]')||event.target===state.modal){event.preventDefault();state.hide();return;}if(event.target.closest('[data-monster-drop-batch-select-filtered]')){for(const choice of state.choices){if(choice.hidden||state.selected.size>=12)continue;state.selected.add(choice.dataset.itemId);}if(state.selected.size>=12)state.error.textContent='一次最多选择 12 件物品';state.apply();return;}if(event.target.closest('[data-monster-drop-batch-clear]')){state.selected.clear();state.error.textContent='';state.apply();}});document.addEventListener('keydown',event=>{if(event.key==='Escape'&&state.modal&&!state.modal.hidden)state.hide();});document.addEventListener('change',event=>{if(!state.modal||!state.modal.contains(event.target))return;if(event.target===state.mode||event.target===state.category||event.target===state.quality)state.apply();});document.addEventListener('input',event=>{if(event.target===state.search)state.apply();});}state.apply();};"
    ...
    "const setupSceneBattleMonsterSelection=()=>{if(document.documentElement.dataset.sceneBattleMonsterSelectionBound==='1')return;document.documentElement.dataset.sceneBattleMonsterSelectionBound='1';document.addEventListener('click',event=>{const link=event.target.closest('#scene-battle-monster-list [data-scene-battle-monster-scene]');if(!link||event.defaultPrevented||event.button!==0||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey||link.target&&link.target!=='_self'||link.hasAttribute('download'))return;const url=new URL(link.href,window.location.href);if(url.origin!==window.location.origin||url.pathname!==window.location.pathname||url.searchParams.get('tab')!=='scene-monsters')return;const list=document.querySelector('#scene-battle-monster-list'),editor=document.querySelector('.grid>.editor'),search=document.querySelector('#scene-battle-monster-search'),content=document.querySelector('#admin-spa-content');if(!list||!editor)return;event.preventDefault();event.stopImmediatePropagation();const query=search?search.value:'',top=list.scrollTop;content?.setAttribute('aria-busy','true');fetch(url,{credentials:'same-origin',cache:'no-store'}).then(response=>{if(!response.ok)throw new Error(`HTTP ${response.status}`);return response.text().then(html=>({html,url:new URL(response.url||url,window.location.href)}));}).then(({html,url:responseUrl})=>{const next=new DOMParser().parseFromString(html,'text/html'),nextList=next.querySelector('#scene-battle-monster-list'),nextEditor=next.querySelector('.grid>.editor');if(!nextList||!nextEditor){window.location.assign(responseUrl);return;}editor.innerHTML=nextEditor.innerHTML;for(const sceneLink of list.querySelectorAll('[data-scene-battle-monster-scene]'))sceneLink.classList.toggle('on',new URL(sceneLink.href,window.location.href).href===responseUrl.href);if(search){search.value=query;search.dispatchEvent(new Event('input'));}list.scrollTop=top;document.title=next.title||document.title;history.pushState(null,'',responseUrl);setupAdminContent();}).catch(()=>{window.location.assign(url);}).finally(()=>content?.removeAttribute('aria-busy'));},true);};"
    ...
    "};"
    "const setupScenePicker=()=>{"
    "const state=window.__cbeScenePickerState||(window.__cbeScenePickerState={bound:false});"
    "const modal=document.querySelector('#scene-picker-modal'),close=document.querySelector('#scene-picker-close'),search=document.querySelector('#scene-picker-search'),list=document.querySelector('#scene-picker-list'),count=document.querySelector('#scene-picker-result-count'),empty=document.querySelector('#scene-picker-empty'),selects=[...document.querySelectorAll('select.scene-resource-select')];"
    "if(!modal||!close||!search||!list||!count||!empty||!selects.length)return;"
    "const roleOperationOpen=()=>!!document.querySelector('[data-role-operation-modal]:not([hidden])');"
    "state.modal=modal;state.close=close;state.search=search;state.list=list;state.count=count;state.empty=empty;state.selects=selects;state.update=select=>{if(!select)return;const field=select.closest('.scene-picker-field'),trigger=field&&field.querySelector('[data-scene-picker-open]'),label=trigger&&trigger.querySelector('[data-scene-picker-label]'),selected=select.options[select.selectedIndex];if(label)label.textContent=selected&&selected.textContent?selected.textContent:(select.dataset.scenePickerEmpty||'请选择目标场景');};"
    "for(const select of selects){if(select.dataset.scenePickerBound!=='1'){select.dataset.scenePickerBound='1';select.addEventListener('change',()=>state.update(select));}state.update(select);}"
    "state.render=()=>{if(!state.active)return;state.list.replaceChildren();state.choices=[];for(const option of state.active.options){if(!option.value||option.disabled)continue;const button=document.createElement('button'),title=document.createElement('strong'),meta=document.createElement('span');button.type='button';button.className='item-choice';button.dataset.search=option.textContent.toLowerCase();button.classList.toggle('selected',option.value===state.active.value);title.textContent=option.textContent;meta.textContent=option.value===state.active.value?'当前选择':'SCE 场景';button.append(title,meta);button.addEventListener('click',()=>{if(!state.active)return;state.active.value=option.value;state.active.dispatchEvent(new Event('change',{bubbles:true}));state.hide();});state.choices.push(button);state.list.append(button);}};"
    "state.apply=()=>{state.render();const keyword=state.search.value.trim().toLowerCase();let shown=0;for(const choice of state.choices){const visible=!keyword||choice.dataset.search.includes(keyword);choice.hidden=!visible;if(visible)shown++;}state.count.textContent=`找到 ${shown} 个场景`;state.empty.hidden=shown!==0;};"
    "state.show=select=>{if(!select||!select.isConnected||select.disabled)return;state.active=select;state.search.value='';state.modal.hidden=false;document.body.classList.add('modal-open');state.apply();state.search.focus();};"
    "state.hide=()=>{if(!state.modal)return;state.modal.hidden=true;if(!roleOperationOpen())document.body.classList.remove('modal-open');const field=state.active&&state.active.closest('.scene-picker-field'),trigger=field&&field.querySelector('[data-scene-picker-open]');if(trigger)trigger.focus();};"
    "if(!state.bound){state.bound=true;document.addEventListener('click',event=>{const trigger=event.target.closest('[data-scene-picker-open]');if(!trigger)return;const field=trigger.closest('.scene-picker-field'),select=field&&field.querySelector('select.scene-resource-select');if(!select||select.disabled)return;event.preventDefault();state.show(select);});document.addEventListener('keydown',event=>{if(event.key==='Escape'&&state.modal&&!state.modal.hidden)state.hide();});}"
    "if(modal.dataset.scenePickerBound!=='1'){modal.dataset.scenePickerBound='1';close.addEventListener('click',()=>state.hide());modal.addEventListener('click',event=>{if(event.target===modal)state.hide();});search.addEventListener('input',()=>state.apply());}"
    "};"
    "const setupMonsterPicker=()=>{"
    "const state=window.__cbeMonsterPickerState||(window.__cbeMonsterPickerState={bound:false});"
    "const source=document.querySelector('#monster-picker-options'),modal=document.querySelector('#monster-picker-modal'),close=document.querySelector('#monster-picker-close'),search=document.querySelector('#monster-picker-search'),list=document.querySelector('#monster-picker-list'),count=document.querySelector('#monster-picker-result-count'),empty=document.querySelector('#monster-picker-empty'),error=document.querySelector('#monster-picker-error'),selects=[...document.querySelectorAll('select.monster-resource-select')];"
    "if(!source||!modal||!close||!search||!list||!count||!empty||!error||!selects.length)return;"
    "const roleOperationOpen=()=>!!document.querySelector('[data-role-operation-modal]:not([hidden])');"
    "state.source=source;state.modal=modal;state.close=close;state.search=search;state.list=list;state.count=count;state.empty=empty;state.error=error;state.selects=selects;state.options=[...source.options].filter(option=>option.value);state.optionByValue=new Map(state.options.map(option=>[option.value,option]));"
    "state.update=select=>{if(!select)return;const field=select.closest('.monster-picker-field'),trigger=field&&field.querySelector('[data-monster-picker-open]'),label=trigger&&trigger.querySelector('[data-monster-picker-label]'),selected=select.options[select.selectedIndex],option=state.optionByValue.get(select.value);if(label)label.textContent=option?option.textContent:(selected&&selected.textContent?selected.textContent:(select.dataset.monsterPickerAllowNone==='1'?'不额外刷新':'请选择怪物'));};"
    "state.setValue=(select,value)=>{if(!select)return;let local=[...select.options].find(option=>option.value===value),option=state.optionByValue.get(value);if(!local){local=document.createElement('option');local.value=value;local.textContent=option?option.textContent:`#${value}`;select.append(local);}select.value=value;select.dispatchEvent(new Event('change',{bubbles:true}));};"
    "for(const select of selects){if(select.dataset.monsterPickerBound!=='1'){select.dataset.monsterPickerBound='1';select.addEventListener('change',()=>state.update(select));}state.update(select);}"
    "state.render=()=>{if(!state.active)return;state.list.replaceChildren();state.choices=[];const addChoice=(value,titleText,metaText)=>{const button=document.createElement('button'),title=document.createElement('strong'),meta=document.createElement('span');button.type='button';button.className='item-choice monster-choice';button.dataset.search=(titleText+' '+metaText).toLowerCase();button.classList.toggle('selected',value===state.active.value);title.textContent=titleText;meta.textContent=metaText;button.append(title,meta);button.addEventListener('click',()=>{if(!state.active)return;state.setValue(state.active,value);state.error.textContent='';state.hide();});state.choices.push(button);state.list.append(button);};if(state.active.dataset.monsterPickerAllowNone==='1')addChoice('0','不额外刷新','进入副本后不新增场景战斗怪');for(const option of state.options)addChoice(option.value,option.textContent,option.value===state.active.value?'当前选择':'怪物目录');};"
    ...
    "const setupTaskRequirementMonsterPicker=()=>{if(document.documentElement.dataset.taskRequirementMonsterPickerBound==='1')return;document.documentElement.dataset.taskRequirementMonsterPickerBound='1';document.addEventListener('click',event=>{const trigger=event.target.closest('[data-task-requirement-target] [data-monster-picker-open]');if(!trigger)return;const target=trigger.closest('[data-task-requirement-target]'),select=target?.querySelector('select.monster-resource-select'),state=window.__cbeMonsterPickerState;if(!select||select.disabled||!state||typeof state.show!=='function')return;event.preventDefault();event.stopPropagation();state.show(select);});};setupTaskRequirementMonsterPicker();"
    "const setupNpcServices=()=>{setupScenePicker();setupNpcTaskListEditor();for(const form of document.querySelectorAll('.npc form')){"
    "const toggles=[...form.querySelectorAll('input[data-npc-service-toggle]')];if(!toggles.length)continue;const card=form.closest('.npc')||form;"
    "const apply=()=>{for(const toggle of toggles){const kind=toggle.dataset.npcServiceToggle,show=toggle.checked;for(const fields of card.querySelectorAll(`[data-npc-service-config=\"${kind}\"]`))fields.hidden=!show;}const teleportToggle=form.querySelector('input[data-npc-service-toggle=\"6\"]'),challengeToggle=form.querySelector('input[data-npc-service-toggle=\"10\"]'),setFields=(selector,show)=>{for(const fields of card.querySelectorAll(selector)){fields.hidden=!show;for(const control of fields.querySelectorAll('input,select,button'))control.disabled=!show;}};setFields('[data-npc-instance-teleport-fields]',!!(teleportToggle&&teleportToggle.checked));setFields('[data-npc-instance-spawn-fields]',!!(teleportToggle&&teleportToggle.checked));setFields('[data-npc-instance-challenge-fields]',!!(challengeToggle&&challengeToggle.checked));setFields('[data-npc-instance-level-fields]',!!((teleportToggle&&teleportToggle.checked)||(challengeToggle&&challengeToggle.checked)));};"
    "for(const toggle of toggles)toggle.addEventListener('change',apply);apply();}};"
    ...
    "const setupOnlinePlayerStat=()=>{const node=document.querySelector('[data-online-player-count]');if(!node||node.dataset.bound==='1')return;node.dataset.bound='1';let loading=false;const refresh=async()=>{if(loading||!document.contains(node))return;loading=true;try{const response=await fetch(new URL('online-players',window.location.href),{credentials:'same-origin',cache:'no-store'});if(!response.ok)throw new Error('HTTP '+response.status);const payload=await response.json(),count=Number(payload.online_players);if(!Number.isSafeInteger(count)||count<0)throw new Error('invalid online-player count');node.textContent=String(count);node.closest('[data-online-player-stat]')?.removeAttribute('data-stale');}catch(error){node.closest('[data-online-player-stat]')?.setAttribute('data-stale','true');}finally{loading=false;}};void refresh();const timer=window.setInterval(()=>{if(!document.contains(node)){window.clearInterval(timer);return;}void refresh();},10000);};"
    ...
    "const setupRoleOperationModal=()=>{const state=window.__cbeRoleOperationState||(window.__cbeRoleOperationState={bound:false,active:null,opener:null,pending:null});const activate=(modal,key)=>{if(!modal||!key)return;for(const pane of modal.querySelectorAll('[data-role-operation-pane]'))pane.hidden=pane.dataset.roleOperationPane!==key;for(const button of modal.querySelectorAll('[data-role-operation-tab]')){const on=button.dataset.roleOperationTab===key;button.classList.toggle('on',on);if(on)button.setAttribute('aria-current','page');else button.removeAttribute('aria-current');}};const close=modal=>{if(!modal)return;modal.hidden=true;if(state.active===modal)state.active=null;document.body.classList.remove('modal-open');const opener=state.opener;state.opener=null;if(opener&&document.contains(opener))opener.focus();};state.restore=(id,key)=>{const modal=document.getElementById(id);if(!modal)return;state.active=modal;state.opener=null;modal.hidden=false;document.body.classList.add('modal-open');activate(modal,key);};if(!state.bound){state.bound=true;document.addEventListener('click',event=>{const opener=event.target.closest('[data-role-operation-open]');if(opener){const modal=document.getElementById(opener.dataset.roleOperationOpen);if(!modal)return;for(const other of document.querySelectorAll('[data-role-operation-modal]'))other.hidden=true;state.active=modal;state.opener=opener;modal.hidden=false;document.body.classList.add('modal-open');const focusTarget=modal.querySelector('[data-role-operation-tab].on')||modal.querySelector('[data-role-operation-tab]');focusTarget?.focus();event.preventDefault();return;}const tab=event.target.closest('[data-role-operation-tab]');if(tab){activate(tab.closest('[data-role-operation-modal]'),tab.dataset.roleOperationTab);tab.focus();return;}const closeButton=event.target.closest('[data-role-operation-close]');if(closeButton){close(closeButton.closest('[data-role-operation-modal]'));return;}const modal=event.target.closest('[data-role-operation-modal]');if(modal&&event.target===modal)close(modal);});document.addEventListener('submit',event=>{const form=event.target,modal=form&&form.closest('[data-role-operation-modal]');if(!modal||form.dataset.adminSpaSubmitting!=='1')return;const key=modal.querySelector('[data-role-operation-tab].on')?.dataset.roleOperationTab;if(modal.id&&key)state.pending={id:modal.id,key};},true);document.addEventListener('keydown',event=>{const itemPicker=document.querySelector('#item-picker-modal'),scenePicker=document.querySelector('#scene-picker-modal');if(event.key==='Escape'&&((itemPicker&&!itemPicker.hidden)||(scenePicker&&!scenePicker.hidden)))return;if(event.key==='Escape'&&state.active&&!state.active.hidden){event.preventDefault();close(state.active);}});}for(const modal of document.querySelectorAll('[data-role-operation-modal]'))activate(modal,modal.querySelector('[data-role-operation-tab]')?.dataset.roleOperationTab);if(state.pending){const pending=state.pending;state.pending=null;state.restore(pending.id,pending.key);}};"
    "const setupAdminContent=()=>{setupGlobalRewardsTab();setupDesignationTab();setupDesignationDirectory();setupRoleOperationModal();setupContentNavigation();setupAdminToasts();setupAccountList();setupOnlinePlayerStat();setupMonsterSearch();setupSceneBattleMonsterSearch();setupMonsterBatchReset();keep('.scene-list','cbe-admin-scenes-scroll');keep('.shop-list','cbe-admin-shop-scroll');keep('.update-menu','cbe-admin-update-menu-scroll');setupItemPicker();setupNpcStock();setupMonsterDrops();setupMonsterDropBatchModal();setupTaskRewards();setupChestRewards();setupGlobalRewards();setupActorPicker();setupMonsterPicker();setupTaskRequirementTargets();setupNpcServices();setupContentUpdatePicker();setupContentResourceSearch();};"
    ...
    "const setupNpcTaskPicker=()=>{const state=window.__cbeNpcTaskPickerState||(window.__cbeNpcTaskPickerState={bound:false,active:null,choices:[]}),refresh=()=>{const modal=document.querySelector('#npc-task-picker-modal'),search=document.querySelector('#npc-task-picker-search'),list=document.querySelector('#npc-task-picker-list'),count=document.querySelector('#npc-task-picker-result-count'),empty=document.querySelector('#npc-task-picker-empty');if(!modal||!search||!list||!count||!empty)return false;state.modal=modal;state.search=search;state.list=list;state.count=count;state.empty=empty;return true;};if(!refresh())return;state.update=select=>{if(!select)return;const field=select.closest('.npc-task-picker-field'),trigger=field&&field.querySelector('[data-npc-task-picker-open]'),label=trigger&&trigger.querySelector('[data-npc-task-picker-label]'),selected=select.options[select.selectedIndex];if(label)label.textContent=selected&&selected.textContent?selected.textContent:(select.value?`未知任务 #${select.value}`:'不绑定任务');};for(const select of document.querySelectorAll('select.npc-task-select')){if(select.dataset.npcTaskPickerBound!=='1'){select.dataset.npcTaskPickerBound='1';select.addEventListener('change',()=>state.update(select));}state.update(select);}state.render=()=>{if(!state.active)return;state.list.replaceChildren();state.choices=[];for(const option of state.active.options){if(option.disabled)continue;const button=document.createElement('button'),title=document.createElement('strong'),meta=document.createElement('span'),selected=option.value===state.active.value;button.type='button';button.className='item-choice npc-task-choice';button.dataset.search=option.textContent.toLocaleLowerCase();button.classList.toggle('selected',selected);title.textContent=option.textContent;meta.textContent=option.value==='0'?'不向此 NPC 绑定任务':(selected?'当前选择':'可接取任务');button.append(title,meta);button.addEventListener('click',()=>{if(!state.active)return;state.active.value=option.value;state.active.dispatchEvent(new Event('change',{bubbles:true}));state.hide();});state.choices.push(button);state.list.append(button);}};state.apply=()=>{state.render();const keyword=state.search.value.trim().toLocaleLowerCase();let shown=0;for(const choice of state.choices){const visible=!keyword||choice.dataset.search.includes(keyword);choice.hidden=!visible;if(visible)shown++;}state.count.textContent=`找到 ${shown} 个任务`;state.empty.hidden=shown!==0;};state.show=select=>{if(!select||!select.isConnected||select.disabled||!refresh())return;state.active=select;state.search.value='';state.modal.hidden=false;document.body.classList.add('modal-open');state.apply();state.search.focus();};state.hide=()=>{if(!state.modal)return;state.modal.hidden=true;document.body.classList.remove('modal-open');const field=state.active&&state.active.closest('.npc-task-picker-field'),trigger=field&&field.querySelector('[data-npc-task-picker-open]');if(trigger)trigger.focus();};if(!state.bound){state.bound=true;document.addEventListener('click',event=>{const trigger=event.target.closest('[data-npc-task-picker-open]');if(trigger){const field=trigger.closest('.npc-task-picker-field'),select=field&&field.querySelector('select.npc-task-select');if(!select||select.disabled)return;event.preventDefault();state.show(select);return;}if(event.target.closest('[data-npc-task-picker-close]')){event.preventDefault();state.hide();return;}if(state.modal&&event.target===state.modal)state.hide();});document.addEventListener('keydown',event=>{if(event.key==='Escape'&&state.modal&&!state.modal.hidden)state.hide();});}if(state.modal.dataset.npcTaskPickerBound!=='1'){state.modal.dataset.npcTaskPickerBound='1';state.search.addEventListener('input',()=>state.apply());}};"
    "const setupNpcTaskListEditor=()=>{const state=window.__cbeNpcTaskListEditorState||(window.__cbeNpcTaskListEditorState={bound:false,active:null,choices:[]}),refresh=()=>{const modal=document.querySelector('#npc-task-picker-modal'),search=document.querySelector('#npc-task-picker-search'),list=document.querySelector('#npc-task-picker-list'),count=document.querySelector('#npc-task-picker-result-count'),empty=document.querySelector('#npc-task-picker-empty');if(!modal||!search||!list||!count||!empty)return false;state.modal=modal;state.search=search;state.list=list;state.count=count;state.empty=empty;return true;},selected=select=>[...select.options].filter(option=>option.selected&&!option.disabled),max=select=>Number(select.dataset.npcTaskMax)||8,stateFor=select=>({field:select.closest('.npc-task-picker-field'),selected:selected(select),max:max(select)});if(!refresh())return;if(!document.querySelector('#npc-task-list-editor-style')){const style=document.createElement('style');style.id='npc-task-list-editor-style';style.textContent='.npc-task-list{display:grid;gap:8px;margin:0 0 8px}.npc-task-list-row{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:9px 10px;border:1px solid #d6dfed;border-radius:8px;background:#f8fbff}.npc-task-list-row span{min-width:0;color:#24476d;overflow-wrap:anywhere}.npc-task-list-row button{flex:0 0 auto}.npc-task-picker-field>[data-npc-task-list-empty]{margin:0 0 8px}.npc-task-picker-trigger{width:100%;justify-content:space-between}';document.head.append(style);}state.sync=select=>{if(!select)return;const {field,selected:chosen,max:limit}=stateFor(select);if(!field)return;const values=field.querySelector('[data-npc-task-values]'),list=field.querySelector('[data-npc-task-list]'),empty=field.querySelector('[data-npc-task-list-empty]'),label=field.querySelector('[data-npc-task-picker-label]');if(values)values.value=chosen.map(option=>option.value).join(',');if(list){list.replaceChildren();for(const option of chosen){const row=document.createElement('div'),name=document.createElement('span'),remove=document.createElement('button');row.className='npc-task-list-row';name.textContent=option.textContent;remove.type='button';remove.className='danger';remove.dataset.npcTaskRemove=option.value;remove.textContent='移除';remove.addEventListener('click',()=>{option.selected=false;select.dispatchEvent(new Event('change',{bubbles:true}));});row.append(name,remove);list.append(row);}}if(empty)empty.hidden=chosen.length!==0;if(label)label.textContent=`＋ 添加任务（${chosen.length}/${limit}）`;};for(const select of document.querySelectorAll('select.npc-task-select')){if(select.dataset.npcTaskListBound!=='1'){select.dataset.npcTaskListBound='1';select.addEventListener('change',()=>state.sync(select));}state.sync(select);}state.render=()=>{if(!state.active)return;state.list.replaceChildren();state.choices=[];for(const option of state.active.options){if(option.disabled)continue;const button=document.createElement('button'),title=document.createElement('strong'),meta=document.createElement('span'),chosen=option.selected;button.type='button';button.className='item-choice npc-task-choice';button.dataset.search=option.textContent.toLocaleLowerCase();button.classList.toggle('selected',chosen);title.textContent=option.textContent;meta.textContent=chosen?'已在任务列表中；点击移除':'点击加入任务列表';button.append(title,meta);button.addEventListener('click',()=>{if(!state.active)return;const selectedNow=selected(state.active);if(!option.selected&&selectedNow.length>=max(state.active)){state.notice=`最多只能选择 ${max(state.active)} 个任务`;state.apply();return;}state.notice='';option.selected=!option.selected;state.active.dispatchEvent(new Event('change',{bubbles:true}));state.apply();});state.choices.push(button);state.list.append(button);}};state.apply=()=>{state.render();const keyword=state.search.value.trim().toLocaleLowerCase();let shown=0;for(const choice of state.choices){const visible=!keyword||choice.dataset.search.includes(keyword);choice.hidden=!visible;if(visible)shown++;}const chosen=state.active?selected(state.active).length:0,limit=state.active?max(state.active):0;state.count.textContent=`找到 ${shown} 个任务 · 已选 ${chosen}/${limit}${state.notice?` · ${state.notice}`:''}`;state.empty.hidden=shown!==0;};state.show=select=>{if(!select||!select.isConnected||select.disabled||!refresh())return;state.active=select;state.notice='';state.search.value='';state.modal.hidden=false;document.body.classList.add('modal-open');state.apply();state.search.focus();};state.hide=()=>{if(!state.modal)return;state.modal.hidden=true;document.body.classList.remove('modal-open');const field=state.active&&state.active.closest('.npc-task-picker-field'),trigger=field&&field.querySelector('[data-npc-task-picker-open]');if(trigger)trigger.focus();};if(!state.bound){state.bound=true;document.addEventListener('click',event=>{const trigger=event.target.closest('[data-npc-task-picker-open]');if(trigger){const field=trigger.closest('.npc-task-picker-field'),select=field&&field.querySelector('select.npc-task-select');if(!select||select.disabled)return;event.preventDefault();state.show(select);return;}if(event.target.closest('[data-npc-task-picker-clear]')){event.preventDefault();if(state.active){for(const option of state.active.options)option.selected=false;state.active.dispatchEvent(new Event('change',{bubbles:true}));state.notice='';state.apply();}return;}if(event.target.closest('[data-npc-task-picker-confirm]')){event.preventDefault();state.hide();return;}if(event.target.closest('[data-npc-task-picker-close]')){event.preventDefault();state.hide();return;}if(state.modal&&event.target===state.modal)state.hide();});document.addEventListener('keydown',event=>{if(event.key==='Escape'&&state.modal&&!state.modal.hidden)state.hide();});}if(state.modal.dataset.npcTaskListBound!=='1'){state.modal.dataset.npcTaskListBound='1';state.search.addEventListener('input',()=>state.apply());}};"
    "document.addEventListener('DOMContentLoaded',()=>queueMicrotask(setupAdminTopbar));"
    "document.addEventListener('DOMContentLoaded',()=>{setupMonsterActions();setupTaskActions();setupPartialNavigation();setupSceneBattleMonsterSelection();setupAdminSpa();setupAdminLayout();setupAdminHeader();setupAdminContent();});"
    "})();";
...
/* XSE remains a supported runtime compatibility path, but content editing now
 * uses the explicit task bindings below.  Preserve an existing script value
 * on unrelated NPC edits without exposing an XSE selector in the UI. */
static void vm_mock_admin_render_npc_script_hidden_input(
    vm_mock_admin_text *page, const char *currentScript)
{
    char scriptUtf8[192];
    ...
    if (currentScript != NULL && currentScript[0] != 0)
    {
        vm_net_mock_gbk_label_to_utf8(currentScript, scriptUtf8,
                                      sizeof(scriptUtf8));
    }
    vm_mock_admin_text_appendf(page,
                                "<input type=\"hidden\" name=\"script_name\" value=\"");
    vm_mock_admin_text_append_html(page, scriptUtf8);
    vm_mock_admin_text_appendf(page, "\">");
}
    ...
    const vm_net_mock_scene_npcinfo_seed *seed, bool allowInstance)
{
         ...
         kind <= VM_NET_MOCK_NPC_KIND_MAX; ++kind)
    {
        ...
        if (!allowInstance &&
            vm_net_mock_npc_service_kind_uses_instance_config((u16)kind))
            continue;
        ...
        /* Existing kind-6 rows represented both operations in one service.
         * Render their configured guard target as the new explicit challenge
         * checkbox, so an unrelated edit cannot discard it on the next save. */
        if (allowInstance && seed != NULL &&
            kind == VM_NET_MOCK_NPC_KIND_INSTANCE_GUIDE &&
            option != NULL && seed->instanceScene[0] == 0 &&
        ...
        if (allowInstance && seed != NULL &&
            ...
            option == NULL && seed->challengeEnemyId != 0 &&
            vm_net_mock_npc_service_options_has_kind(
                options, optionCount, VM_NET_MOCK_NPC_KIND_INSTANCE_GUIDE))
        {
            selected = true;
        }
        ...
        vm_mock_admin_text_appendf(
            page,
            "<div class=\"npc-service-row\"><label class=\"npc-service-toggle\"><input type=\"checkbox\" data-npc-service-toggle=\"%u\" name=\"service_enabled_%u\" value=\"1\"%s><span>%u · %s</span></label><div class=\"npc-service-config\" data-npc-service-config=\"%u\"%s><label class=\"field\"><span>选项名称（可留空）</span><input name=\"service_option_name_%u\" maxlength=\"30\" placeholder=\"默认名称\" value=\"",
            kind, kind, selected ? " checked" : "", kind, labels[kind], kind,
        ...
        vm_mock_admin_text_appendf(page, "\"></label></div></div>");
    }
    ...
    const char *currentRuntimeScene)
{
    ...
    vm_mock_admin_text_appendf(
        page,
        "<div class=\"scene-picker-field\"><select class=\"scene-resource-select\" name=\"instance_scene\" hidden><option value=\"\">请选择副本目标场景</option>");
    ...
    vm_mock_admin_text_appendf(
        page,
        "</select><button class=\"item-picker-trigger scene-picker-trigger\" type=\"button\" data-scene-picker-open aria-haspopup=\"dialog\" aria-controls=\"scene-picker-modal\"><span data-scene-picker-label>请选择副本目标场景</span><small>搜索场景</small></button></div>");
}

/* A single dialog is shared by every actual SCE <select> on administrative
 * pages. Each trigger keeps its authoritative select in the form, so server
 * validation and pending form submissions retain their existing contract. */
static void vm_mock_admin_render_scene_picker_modal(vm_mock_admin_text *page)
{
    if (page == NULL)
        return;
    vm_mock_admin_text_appendf(
        page,
        "<div class=\"item-modal scene-picker-modal\" id=\"scene-picker-modal\" role=\"dialog\" aria-modal=\"true\" aria-labelledby=\"scene-picker-title\" hidden><section class=\"item-picker-panel\"><div class=\"item-picker-head\"><div><h3 id=\"scene-picker-title\">选择 SCE 场景</h3><p>按场景文件名搜索；选择后会更新当前表单的目标场景。</p></div><button class=\"item-picker-close\" id=\"scene-picker-close\" type=\"button\" aria-label=\"关闭场景选择\">×</button></div><div class=\"item-picker-tools\"><label><span>搜索场景</span><input id=\"scene-picker-search\" type=\"search\" placeholder=\"输入场景名称或文件名\" autocomplete=\"off\"></label></div><div class=\"item-result-bar\"><span id=\"scene-picker-result-count\"></span></div><div class=\"item-picker-list\" id=\"scene-picker-list\"></div><p class=\"item-picker-empty\" id=\"scene-picker-empty\" hidden>没有符合条件的 SCE 场景。</p></section></div>");
}

/* The dialog and its catalog are rendered once per page.  Individual forms
 * retain only their selected value, so NPC-heavy scenes do not duplicate the
 * whole monster catalog for every instance/challenge field. */
static void vm_mock_admin_render_monster_picker_field(
    vm_mock_admin_text *page, const char *fieldName, u32 monsterId,
    bool required, bool allowNone, bool disabled);

/* Role recovery accepts only an exact server-owned SCE key.  Keep that
 * authoritative value in a hidden select and let the shared scene picker
 * provide the searchable UI above the role-operation modal. */
static void vm_mock_admin_render_role_reset_scene_picker_field(
    vm_mock_admin_text *page,
    const vm_mock_admin_scene_file *sceneFiles,
    u32 sceneCount)
{
    ...
    vm_mock_admin_text_appendf(
        page,
        "<div class=\"scene-picker-field\"><select class=\"scene-resource-select\" name=\"reset_scene\" required hidden><option value=\"\">请选择目标场景</option>");
    for (u32 i = 0; i < sceneCount; ++i)
    {
        char runtimeScene[64];
        char sceneUtf8[192];
    ...
    vm_mock_admin_text_appendf(
        page,
        "</select><button class=\"item-picker-trigger scene-picker-trigger\" type=\"button\" data-scene-picker-open aria-haspopup=\"dialog\" aria-controls=\"scene-picker-modal\"><span data-scene-picker-label>请选择目标场景</span><small>搜索场景</small></button></div>");
}
    ...
    const vm_net_mock_scene_npcinfo_seed *seed)
{
    ...
    vm_mock_admin_text_appendf(
        page,
        "<div class=\"instance-fields\" data-npc-service-config=\"6\" data-npc-instance-teleport-fields hidden><label class=\"field instance-scene\"><span>副本目标场景</span>");
    ...
    vm_mock_admin_text_appendf(
        page,
        "</label><p class=\"instance-help\">挑战对象必须是当前场景中已启用的场景战斗怪；选择后客户端会直接发起战斗，不显示二次确认。</p></div>"
        "<div class=\"instance-fields\" data-npc-instance-level-fields hidden><label class=\"field\"><span>副本功能最低等级</span><input type=\"number\" name=\"instance_min_level\" min=\"1\" max=\"255\" value=\"%u\"></label>"
        "<p class=\"instance-help\">该等级限制同时用于已启用的副本传送和守关怪挑战。</p></div>",
        minimumLevel);
}

static bool vm_mock_admin_npc_task_selected(
    const vm_net_mock_dynamic_npc_task_binding *bindings, u32 bindingCount,
    u32 taskId)
{
    if (bindings == NULL)
        return false;
    ...
    {
        if (bindings[index].taskId == taskId)
            return true;
    }
    return false;
}

static void vm_mock_admin_render_npc_task_select(
    vm_mock_admin_text *page,
    const vm_net_mock_dynamic_npc_task_binding *bindings, u32 bindingCount)
{
    vm_net_mock_task_definition tasks[VM_NET_MOCK_TASK_CATALOG_MAX];
    u32 taskCount = 0;
    char taskIds[(VM_NET_MOCK_DYNAMIC_NPC_TASK_MAX * 11u) + 1u];
    size_t taskIdsLen = 0;

    if (page == NULL || bindingCount > VM_NET_MOCK_DYNAMIC_NPC_TASK_MAX)
        return;
    ...
    for (u32 index = 0; index < bindingCount; ++index)
    {
        ...
        if (bindings == NULL || bindings[index].taskId == 0 ||
            vm_mock_admin_npc_task_selected(bindings, index,
                                            bindings[index].taskId))
        {
            return;
        }
        written = snprintf(taskIds + taskIdsLen, sizeof(taskIds) - taskIdsLen,
                           "%s%u", taskIdsLen != 0 ? "," : "",
                           bindings[index].taskId);
        if (written < 0 || (size_t)written >= sizeof(taskIds) - taskIdsLen)
            return;
        taskIdsLen += (size_t)written;
    }
    taskCount = vm_net_mock_task_admin_list(
        tasks, VM_NET_MOCK_TASK_CATALOG_MAX);
    vm_mock_admin_text_appendf(
        page, "<div class=\"npc-task-picker-field\" data-npc-task-list-editor><select class=\"npc-task-select\" data-npc-task-max=\"%u\" multiple hidden>",
    ...
    for (u32 i = 0; i < taskCount; ++i)
    {
        ...
        vm_mock_admin_text_appendf(
            ...
            vm_mock_admin_npc_task_selected(bindings, bindingCount,
                                            tasks[i].taskId) ? " selected" : "",
            tasks[i].taskId);
        vm_mock_admin_text_append_html(page, nameUtf8);
    ...
    vm_mock_admin_text_appendf(
        page,
        "</select><input type=\"hidden\" name=\"task_ids\" data-npc-task-values value=\"");
    vm_mock_admin_text_append_html(page, taskIds);
    vm_mock_admin_text_appendf(
        page,
        "\"><div class=\"npc-task-list\" data-npc-task-list></div><p class=\"muted\" data-npc-task-list-empty hidden>尚未绑定可接取任务。</p><button class=\"item-picker-trigger npc-task-picker-trigger\" type=\"button\" data-npc-task-picker-open aria-haspopup=\"dialog\" aria-controls=\"npc-task-picker-modal\"><span data-npc-task-picker-label>＋ 添加任务</span><small>搜索并加入列表</small></button><small>最多 %u 个任务；可在列表中随时移除。</small></div>",
        VM_NET_MOCK_DYNAMIC_NPC_TASK_MAX);
}

static void vm_mock_admin_render_npc_task_picker_modal(
    vm_mock_admin_text *page)
{
    if (page == NULL)
        return;
    vm_mock_admin_text_appendf(
        page,
        "<div class=\"item-modal npc-task-picker-modal\" id=\"npc-task-picker-modal\" role=\"dialog\" aria-modal=\"true\" aria-labelledby=\"npc-task-picker-title\" hidden><section class=\"item-picker-panel\"><div class=\"item-picker-head\"><div><h3 id=\"npc-task-picker-title\">添加可接取任务</h3><p>点击任务可加入或移出列表；可连续选择多个任务。重复接取规则会统一应用到已选任务。</p></div><button class=\"item-picker-close\" type=\"button\" data-npc-task-picker-close aria-label=\"关闭任务选择\">×</button></div><div class=\"item-picker-tools\"><label><span>搜索任务</span><input id=\"npc-task-picker-search\" type=\"search\" placeholder=\"输入任务 ID、名称、等级或前置任务\" autocomplete=\"off\"></label></div><div class=\"item-result-bar\"><span id=\"npc-task-picker-result-count\"></span></div><div class=\"item-picker-list\" id=\"npc-task-picker-list\"></div><p class=\"item-picker-empty\" id=\"npc-task-picker-empty\" hidden>没有符合条件的任务。</p><div class=\"actions\"><button type=\"button\" data-npc-task-picker-clear>清空列表</button><button type=\"button\" data-npc-task-picker-confirm>完成选择</button></div></section></div>");
}
...
typedef struct
{
    ...
    u32 right;
    u32 bottom;
    /* Exact decoded-SCE locations of the u8 string lengths.  They are
     * captured while parsing the record so editors can replace only the
     * verified field and never reconstruct or guess the rest of a portal
     * record. */
    u32 targetLengthOffset;
    u32 displayNameLengthOffset;
    u32 recordOffset;
...
/* Locate one already-validated string within its parsed record.  The portal
 * parsers deliberately describe several native record variants; this narrow
 * capture step keeps every editable field tied to the same descriptor that
 * the client will later parse. */
static bool vm_mock_admin_capture_portal_string_location(
    const u8 *data, u32 len, u32 recordOffset, u32 recordEnd, u16 field,
    const char *expected, u32 *lengthOffsetOut)
{
    size_t expectedLen = 0;

    if (lengthOffsetOut)
        *lengthOffsetOut = 0;
    if (data == NULL || expected == NULL || lengthOffsetOut == NULL ||
        recordOffset >= recordEnd || recordEnd > len)
    {
        return false;
    }
    expectedLen = strlen(expected);
    if (expectedLen == 0 || expectedLen > 0xffu)
        return false;
    for (u32 pos = recordOffset; pos + 5u <= recordEnd; ++pos)
    {
        ...
        if (vm_net_mock_read_le16_at(data, pos) != 3u ||
            vm_net_mock_read_le16_at(data, pos + 2u) != field ||
            valueLen != expectedLen || valueOffset + 1u + valueLen > recordEnd ||
            memcmp(data + valueOffset + 1u, expected, expectedLen) != 0)
        {
            continue;
        }
        *lengthOffsetOut = valueOffset;
        return true;
    }
    return false;
}
    ...
    vm_mock_admin_scene_portal *portal)
{
    ...
    if (!vm_mock_admin_capture_portal_string_location(
            data, len, recordOffset, recordEnd, targetField,
            portal->targetScene, &portal->targetLengthOffset))
    {
        return false;
    }
...
static bool vm_mock_admin_capture_portal_display_name_location(
    const u8 *data, u32 len, u32 recordOffset, u32 recordEnd,
    vm_mock_admin_scene_portal *portal)
{
    return portal != NULL && portal->kind == VM_MOCK_ADMIN_PORTAL_NAMED &&
           vm_mock_admin_capture_portal_string_location(
               data, len, recordOffset, recordEnd, 0x16u,
               portal->displayName, &portal->displayNameLengthOffset);
}
    ...
    u32 *totalOut)
{
    ...
    for (u32 off = start; off + 12 <= len; ++off)
    {
        ...
        if (parsed)
        {
            ...
            if (portal.kind == VM_MOCK_ADMIN_PORTAL_NAMED &&
                !vm_mock_admin_capture_portal_display_name_location(
                    data, len, off, end, &portal))
            {
                /* The catalogue may still identify this record elsewhere,
                 * but no title editor is exposed without an exact field
                 * position in the current decoded SCE. */
                if (end > off + 1)
                    off = end - 1;
                continue;
            }
...
/* A named SCE portal's field 0x16 is the scene-facing location label used by
 * the original data.  The content catalog shows that label as the primary
 * name, while retaining the exact SCE resource key separately for edits and
 * diagnostics.  Do not derive a title from the filename: prefixes and scene
 * ordinals are resource identifiers, not a player-facing location contract. */
static bool vm_mock_admin_scene_location_title(const char *scene,
                                               char *titleOut,
                                               size_t titleOutCap)
{
    ...
    if (titleOut == NULL || titleOutCap == 0)
        return false;
    titleOut[0] = 0;
    ...
    portalCount = vm_mock_admin_collect_scene_portals(
        scene, portals, VM_MOCK_ADMIN_PREVIEW_PORTAL_MAX, NULL);
    for (u32 i = 0; i < portalCount; ++i)
    {
        if (portals[i].kind != VM_MOCK_ADMIN_PORTAL_NAMED ||
            portals[i].displayName[0] == 0 ||
            strlen(portals[i].displayName) >= titleOutCap)
        {
            continue;
        }
        snprintf(titleOut, titleOutCap, "%s", portals[i].displayName);
        return true;
    }
    return false;
}

static void vm_mock_admin_render_scene_catalog_item(
    vm_mock_admin_text *page, const vm_mock_admin_scene_file *sceneFile,
    const char *title, bool selected, bool portalSection)
{
    char sceneUtf8[192];
    char titleUtf8[192];
    char encoded[512];

    if (page == NULL || sceneFile == NULL || sceneFile->name[0] == 0)
        return;
    memset(sceneUtf8, 0, sizeof(sceneUtf8));
    memset(titleUtf8, 0, sizeof(titleUtf8));
    memset(encoded, 0, sizeof(encoded));
    vm_net_mock_gbk_label_to_utf8(sceneFile->name, sceneUtf8,
                                  sizeof(sceneUtf8));
    vm_net_mock_gbk_label_to_utf8(title && title[0] ? title : sceneFile->name,
                                  titleUtf8, sizeof(titleUtf8));
    vm_mock_admin_url_encode(sceneUtf8, encoded, sizeof(encoded));
    vm_mock_admin_text_appendf(
        page,
        selected ?
            "<a id=\"selected-scene\" class=\"scene on\" data-admin-select data-content-resource-item data-content-resource-search-key=\"" :
            "<a class=\"scene\" data-admin-select data-content-resource-item data-content-resource-search-key=\"");
    vm_mock_admin_text_append_html(page, titleUtf8);
    vm_mock_admin_text_appendf(page, " ");
    vm_mock_admin_text_append_html(page, sceneUtf8);
    vm_mock_admin_text_appendf(
        page,
        "\"%s href=\"/?tab=content&amp;content_kind=sce&amp;content_section=%s&amp;scene=%s#selected-scene\"><span class=\"scene-label\"><strong class=\"scene-title\">",
        selected ? " aria-current=\"page\"" : "",
        portalSection ? "portals" : "npcs", encoded);
    vm_mock_admin_text_append_html(page, titleUtf8);
    vm_mock_admin_text_appendf(page,
                               "</strong><small class=\"scene-file\">");
    vm_mock_admin_text_append_html(page, sceneUtf8);
    vm_mock_admin_text_appendf(
        page, "</small></span><span class=\"size\">%llu B</span></a>",
        (unsigned long long)sceneFile->size);
}
...
/* Scene-battle SCE deployment uses this same validator to turn an Actor
 * reference into its complete leaf-resource closure.  Keep the parser in the
 * admin/resource layer: it validates the precise compressed Actor payload and
 * its GIF files, while the scene layer only owns publication order. */
static bool vm_net_mock_actor_resource_collect_images(
    const char *actorResource,
    char imageNames[VM_NET_MOCK_ACTOR_RESOURCE_IMAGE_MAX][64],
    u32 *imageCountOut)
{
    char inspectedNames[VM_MOCK_ADMIN_PREVIEW_IMAGE_MAX][64];
    u32 imageCount = 0;

    if (imageCountOut)
        *imageCountOut = 0;
    if (actorResource == NULL || imageNames == NULL || imageCountOut == NULL ||
        VM_NET_MOCK_ACTOR_RESOURCE_IMAGE_MAX <
            VM_MOCK_ADMIN_PREVIEW_IMAGE_MAX)
    {
        return false;
    }
    memset(inspectedNames, 0, sizeof(inspectedNames));
    memset(imageNames, 0,
           sizeof(imageNames[0]) * VM_NET_MOCK_ACTOR_RESOURCE_IMAGE_MAX);
    if (!vm_mock_admin_actor_resource_inspect(actorResource, inspectedNames,
                                              &imageCount) ||
        imageCount == 0 ||
        imageCount > VM_NET_MOCK_ACTOR_RESOURCE_IMAGE_MAX)
    {
        return false;
    }
    memcpy(imageNames, inspectedNames, sizeof(imageNames[0]) * imageCount);
    *imageCountOut = imageCount;
    return true;
}
...
static void vm_mock_admin_render_monster_picker_modal(vm_mock_admin_text *page);
...
static void vm_mock_admin_render_npc_inventory_setup_pending(
    vm_mock_admin_text *page, bool newNpc, u32 configuredServices);
    ...
    const char *kindLabel, bool gifMode)
{
    ...
    vm_mock_admin_text_appendf(&page,
        ...
        "<nav class=\"tabs\"><a class=\"tab\" href=\"/?tab=accounts\">账号管理</a><a class=\"tab on\" href=\"/?tab=content\">游戏内容管理</a><a class=\"tab\" href=\"/?tab=tasks\">任务管理</a><a class=\"tab\" href=\"/?tab=monsters\">怪物管理</a><a class=\"tab\" href=\"/?tab=updates\">游戏内容更新管理</a></nav>"
        "<div class=\"grid\"><aside class=\"card catalog\"><h2>文件格式</h2><nav class=\"format-tabs\"><a class=\"format\" href=\"/?tab=content&amp;content_kind=sce\">SCE 场景</a><a class=\"format%s\" href=\"/?tab=content&amp;content_kind=gif\">GIF 图片</a><a class=\"format%s\" href=\"/?tab=content&amp;content_kind=dsh\">DSH 数据</a><a class=\"format\" href=\"/?tab=content&amp;content_kind=actor\">Actor 资源</a></nav><h2>%s（%u）</h2><label style=\"display:grid;gap:4px;margin:0 0 10px\"><span class=\"hint\">按名称搜索</span><input type=\"search\" data-content-resource-search placeholder=\"输入资源名称\" autocomplete=\"off\"><small class=\"hint\" data-content-resource-search-count></small></label><div class=\"list\" data-admin-list data-content-resource-list>",
    ...
    for (u32 i = 0; i < count; ++i)
    {
        ...
        vm_mock_admin_text_appendf(&page,
            "<a class=\"file%s\" data-admin-select data-content-resource-item href=\"/?tab=content&amp;content_kind=%s&amp;resource=%s\"><span>",
    ...
    for (u32 i = 0; i < portalCount; ++i)
    {
        ...
        if (portal->entryId == 0xffffu)
            ...
            vm_mock_admin_text_appendf(page, "入口 %u → %u", portal->entryId,
                                       portal->targetEntryId);
        vm_mock_admin_text_appendf(
            page, " · 区域 (%u,%u)-(%u,%u)</span></div><label class=\"field\"><span>目标 SCE</span><div class=\"scene-picker-field\"><select class=\"scene-resource-select\" name=\"target_scene\" required hidden>",
        ...
        vm_mock_admin_text_appendf(
            page,
            "</select><button class=\"item-picker-trigger scene-picker-trigger\" type=\"button\" data-scene-picker-open aria-haspopup=\"dialog\" aria-controls=\"scene-picker-modal\"><span data-scene-picker-label>选择目标 SCE</span><small>搜索场景</small></button></div></label><div class=\"actions\"><button type=\"submit\">保存目标场景</button></div></form>");
    }
    if (portalCount == 0)
        vm_mock_admin_text_appendf(page,
...
/* The SCE resource key, the SCE2 MAP reference and the player-facing location
 * title are independent contracts.  This editor intentionally exposes only
 * two safe operations: byte-preserving copy to a new .sce key, and replacement
 * of the first named portal's verified field-0x16 location title. */
static void vm_mock_admin_render_sce_resource_editor(
    vm_mock_admin_text *page, const char *sceneUtf8, const char *runtimeScene)
{
    char titleGbk[64];
    char titleUtf8[192];
    ...
    if (page == NULL || sceneUtf8 == NULL || sceneUtf8[0] == 0 ||
        runtimeScene == NULL || runtimeScene[0] == 0)
    {
        return;
    }
    memset(titleGbk, 0, sizeof(titleGbk));
    memset(titleUtf8, 0, sizeof(titleUtf8));
    titleEditable = vm_mock_admin_scene_location_title(
        runtimeScene, titleGbk, sizeof(titleGbk));
    if (titleEditable)
        vm_net_mock_gbk_label_to_utf8(titleGbk, titleUtf8, sizeof(titleUtf8));

    vm_mock_admin_text_appendf(
        page,
        "<div class=\"native-section\"><div class=\"native-title\"><h2>SCE 资源管理</h2><span class=\"badge\">原始资源 + 内容更新</span></div>"
        "<p class=\"native-meta\">文件名是客户端请求的 SCE 资源键；复制会保留 SCE2 的 MAP 引用、NPC 和传送点，原文件不会改动。新文件会加入内容更新清单。</p>"
        "<form method=\"post\" action=\"/action\" class=\"portal-row\"><input type=\"hidden\" name=\"action\" value=\"copy-sce-resource\"><input type=\"hidden\" name=\"scene\" value=\"");
    vm_mock_admin_text_append_html(page, sceneUtf8);
    vm_mock_admin_text_appendf(
        page,
        "\"><div><strong>复制为新的 SCE 文件</strong><span class=\"size\">新文件名必须以 .sce 结尾，且不能覆盖已有资源</span></div>"
        "<label class=\"field\"><span>新 SCE 文件名</span><input name=\"new_scene_file\" maxlength=\"63\" placeholder=\"例如 my_scene.sce\" required></label>"
        "<div class=\"actions\"><button type=\"submit\">复制并发布</button></div></form>");
    if (titleEditable && titleUtf8[0] != 0)
    {
        vm_mock_admin_text_appendf(
            page,
            "<form method=\"post\" action=\"/action\" class=\"portal-row\" style=\"margin-top:10px\"><input type=\"hidden\" name=\"action\" value=\"save-sce-location-title\"><input type=\"hidden\" name=\"scene\" value=\"");
        vm_mock_admin_text_append_html(page, sceneUtf8);
        vm_mock_admin_text_appendf(
            page,
            "\"><div><strong>内置场景名称</strong><span class=\"size\">首个具名传送点的 field 0x16；不会改动 SCE2 的 MAP 引用</span></div>"
            "<label class=\"field\"><span>地点显示名称</span><input name=\"scene_title\" maxlength=\"63\" value=\"");
        vm_mock_admin_text_append_html(page, titleUtf8);
        vm_mock_admin_text_appendf(
            page,
            "\" required></label><div class=\"actions\"><button type=\"submit\">保存名称并发布</button></div></form>");
    }
    else
    {
        vm_mock_admin_text_appendf(
            page,
            "<p class=\"foot\">该 SCE 没有可精确定位的具名传送点 field 0x16，因此已禁用内置场景名称编辑；复制仍可使用。</p>");
    }
    vm_mock_admin_text_appendf(page, "</div>");
}
                                              ...
                                              const char *query)
{
    ...
    vm_mock_admin_text_appendf(&page,
        ...
        ".scene-list{display:flex;flex:1;min-height:0;flex-direction:column;gap:4px;overflow-y:auto;overscroll-behavior:contain;scrollbar-gutter:stable;padding-right:4px}.scene{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:8px 9px;border-radius:6px;color:#344054;text-decoration:none;scroll-margin-block:12px}.scene:hover,.scene.on{background:#eef4ff;color:#175cd3}.scene-label{display:grid;min-width:0;gap:1px}.scene-title{overflow-wrap:anywhere}.scene-file{color:#98a2b3;font-size:12px;overflow-wrap:anywhere}.size{color:#98a2b3;font-size:12px;white-space:nowrap}"
        ...
        "<div class=\"grid\"><aside class=\"card\"><h2>文件格式</h2><nav class=\"format-tabs\"><a class=\"format on\" href=\"/?tab=content&amp;content_kind=sce\">SCE 场景</a><a class=\"format\" href=\"/?tab=content&amp;content_kind=gif\">GIF 图片</a><a class=\"format\" href=\"/?tab=content&amp;content_kind=dsh\">DSH 数据</a><a class=\"format\" href=\"/?tab=content&amp;content_kind=actor\">Actor 资源</a></nav><h2>SCE 场景（%u）</h2><label style=\"display:grid;gap:4px;margin:0 0 10px\"><span class=\"foot\">按名称搜索</span><input type=\"search\" data-content-resource-search placeholder=\"输入场景名称\" autocomplete=\"off\"><small class=\"foot\" data-content-resource-search-count></small></label><div class=\"scene-list\" data-admin-list data-content-resource-list>",
        sceneCount);
    for (u32 i = 0; i < sceneCount; ++i)
    {
        char sceneTitle[64];

        memset(sceneTitle, 0, sizeof(sceneTitle));
        if (!vm_mock_admin_scene_location_title(sceneFiles[i].name,
                                                sceneTitle,
                                                sizeof(sceneTitle)))
        {
            snprintf(sceneTitle, sizeof(sceneTitle), "%s",
                     sceneFiles[i].name);
        }
        vm_mock_admin_render_scene_catalog_item(
            &page, &sceneFiles[i], sceneTitle,
            strcmp(sceneFiles[i].name, selectedSceneFile) == 0,
            portalSection);
    }
    if (sceneCount == 0)
    ...
    vm_mock_admin_render_sce_resource_editor(&page, selectedSceneUtf8,
                                             runtimeScene);
    if (previewReady)
    {
    ...
    for (u32 i = 0; i < npcCount; ++i)
    {
        ...
        vm_mock_admin_text_appendf(&page,
            "");
        vm_mock_admin_render_npc_script_hidden_input(&page,
                                                      row->seed.scriptName);
        vm_mock_admin_text_appendf(&page,
            "<div class=\"npc-task-settings\"><label class=\"field\"><span>可接取任务（最多 8 个；会校验等级与前置任务）</span>");
        vm_mock_admin_render_npc_task_select(&page, row->taskBindings,
                                              row->taskBindingCount);
        vm_mock_admin_text_appendf(&page,
            "</label><label class=\"field\"><span>任务重复接取规则（应用到所有已选任务）</span>");
    ...
    vm_mock_admin_text_appendf(&page,
        "");
    vm_mock_admin_render_npc_script_hidden_input(&page, NULL);
    vm_mock_admin_text_appendf(&page,
        "<div class=\"npc-task-settings\"><label class=\"field\"><span>可接取任务（最多 8 个；会校验等级与前置任务）</span>");
    vm_mock_admin_render_npc_task_select(&page, NULL, 0);
    vm_mock_admin_text_appendf(&page,
        "</label><label class=\"field\"><span>任务重复接取规则（应用到所有已选任务）</span>");
    ...
    vm_mock_admin_text_appendf(&page,
        "</div><div class=\"actions npc-editor-save\"><button type=\"submit\">增加 NPC</button></div></form>");
    vm_mock_admin_render_npc_inventory_setup_pending(&page, true, 0);
    vm_mock_admin_text_appendf(&page,
        "</div>"
        "<p class=\"foot\">对话服务功能决定客户端可操作入口；自定义名称和说明只改变该入口的显示文字。武器商人先按剑、匕首、法杖分类；防具商人提供头盔、衣甲、披风、腰带、护腿、鞋靴和戒指；药品商人提供 item.dsh 类别 10 的药品与消耗品。装备回收商人仅列出背包中的装备，并按装备基础价值的 50% 回收为铜钱；已装备在角色身上的物品不会出现在回收列表。副本传送和守关怪挑战可独立勾选：前者直接进入目标场景，后者直接开始当前场景的守关战斗，均按最低等级拦截。比武擂台会打开客户端原生擂台大厅，可开设、查看和加入在线擂台房间；房间随房主离线即时关闭。商品价格和上架状态均来自后台商品目录。装备修理按实际耐久收费；技能导师只列出当前职业、等级可学且尚未学习的技能。SCE 文件中的内置 NPC 不会被改写。客户端同场景最多安全显示 4 个动态名称，超出时仍按任务优先级筛选。</p>"
        "</div>");
    ...
    u32 actorId, u16 serviceKind, const char *pickerPrefix)
{
    ...
    vm_mock_admin_text_appendf(page,
        "<div class=\"inventory npc-stock-manager\" data-npc-stock-manager data-npc-service-config=\"%u\" data-npc-stock-service=\"%u\" data-npc-stock-key=\"%s-%u\"><div class=\"inventory-head\"><h4>NPC 专属库存（%u）</h4><span class=\"stock-note\">仅显示当前服务类型可售的分类</span></div>",
        serviceKind, serviceKind, pickerPrefix, actorId, serviceCount);
    vm_mock_admin_text_appendf(page,
...
/* Store inventory is a second, separately authorized POST workflow.  Show a
 * lightweight configuration region immediately when an administrator checks
 * a new store service, but do not expose submit controls until that service
 * has been saved as part of the NPC's authoritative service set. */
static void vm_mock_admin_render_npc_inventory_setup_pending(
    vm_mock_admin_text *page, bool newNpc, u32 configuredServices)
{
    static const struct
    {
        ...
        {VM_NET_MOCK_NPC_KIND_WEAPON_MERCHANT, "武器商店"},
        {VM_NET_MOCK_NPC_KIND_ARMOR_MERCHANT, "防具商店"},
        {VM_NET_MOCK_NPC_KIND_MEDICINE_MERCHANT, "药品商店"}
    };

    if (page == NULL)
        return;
    for (u32 i = 0; i < sizeof(stores) / sizeof(stores[0]); ++i)
    {
        if (!newNpc && (configuredServices & (1u << stores[i].kind)) != 0)
            continue;
        vm_mock_admin_text_appendf(
            page,
            "<div class=\"inventory npc-inventory-setup\" data-npc-service-config=\"%u\" hidden><div class=\"inventory-head\"><h4>%s 专属库存</h4><span class=\"stock-note\">服务配置</span></div><p class=\"foot\">%s</p></div>",
            stores[i].kind, stores[i].label,
...
/* Requirement IDs are polymorphic.  Collect-item requirements keep the
 * catalog button below; kill-monster requirements use the shared monster
 * dialog rendered by the task page. */
...
static void vm_mock_admin_render_task_requirement_select(
    vm_mock_admin_text *page, const char *name, u8 current, u8 slot);

static void vm_mock_admin_render_task_requirement_target_field(
    vm_mock_admin_text *page, u8 slot, u8 requirementType,
    u32 requirementId)
{
    char targetId[48];
    char fieldName[48];

    if (page == NULL || slot == 0 || slot > 2)
        return;
    snprintf(targetId, sizeof(targetId), "task-requirement-%u", slot);
    snprintf(fieldName, sizeof(fieldName), "requirement_id%u", slot);
    vm_mock_admin_text_appendf(
        page,
        "<div class=\"field\" data-task-requirement-target=\"%u\"><span>目标对象</span><input id=\"%s\" type=\"number\" name=\"%s\" min=\"0\" max=\"4294967295\" value=\"%u\" data-item-picker-input data-task-requirement-numeric%s hidden>",
        slot, targetId, fieldName, requirementId,
        requirementType == 2 ? " disabled" : "");
    vm_mock_admin_text_appendf(
        page, "<span data-task-requirement-item-picker%s>",
        requirementType == 1 ? "" : " hidden");
    vm_mock_admin_render_item_picker_button(page, targetId, "从物品目录选择");
    vm_mock_admin_text_appendf(
        page, "</span><div data-task-requirement-monster%s>",
        requirementType == 2 ? "" : " hidden");
    vm_mock_admin_render_monster_picker_field(
        page, fieldName, requirementType == 2 ? requirementId : 0, false,
        false, requirementType != 2);
    vm_mock_admin_text_appendf(page, "</div></div>");
}

static void vm_mock_admin_render_task_requirement_row(
    vm_mock_admin_text *page, u8 slot, u8 requirementType,
    u32 requirementId, u8 requirementCount, const char *sceneUtf8)
{
    char typeName[48];
    char countName[48];
    char sceneName[48];
    ...
    if (page == NULL || slot == 0 || slot > 2)
        return;
    snprintf(typeName, sizeof(typeName), "requirement_type%u", slot);
    snprintf(countName, sizeof(countName), "requirement_count%u", slot);
    snprintf(sceneName, sizeof(sceneName), "requirement_scene%u", slot);
    vm_mock_admin_text_appendf(
        page,
        "<article class=\"task-target-row\" data-task-requirement-row=\"%u\"%s><div class=\"task-target-head\"><strong data-task-requirement-index>任务目标 %u</strong><button class=\"secondary task-target-remove\" type=\"button\" data-task-requirement-remove>移除</button></div><div class=\"task-target-fields\"><label class=\"field\"><span>目标类型</span>",
        slot, requirementType == 0 ? " hidden" : "", slot);
    vm_mock_admin_render_task_requirement_select(
        page, typeName, requirementType, slot);
    vm_mock_admin_text_appendf(page, "</label>");
    vm_mock_admin_render_task_requirement_target_field(
        page, slot, requirementType, requirementId);
    vm_mock_admin_text_appendf(
        page,
        "<label class=\"field\"><span>数量</span><input type=\"number\" name=\"%s\" min=\"0\" max=\"255\" value=\"%u\" data-task-requirement-count></label><label class=\"field task-target-scene\"><span>限定场景（可选）</span><input name=\"%s\" maxlength=\"63\" value=\"",
        countName, requirementCount, sceneName);
    vm_mock_admin_text_append_html(page, sceneUtf8 ? sceneUtf8 : "");
    vm_mock_admin_text_appendf(
        page,
        "\" placeholder=\"scene.sce（留空表示任意场景）\" data-task-requirement-scene></label></div></article>");
}
...
enum
{
    VM_MOCK_ADMIN_ROLE_TIMED_EFFECT_MAX = 6
};
...
typedef struct
{
    ...
    u8 multiplier;
    u32 expiresUnix;
} vm_mock_admin_role_timed_effect;

typedef struct
{
    vm_mock_admin_role_timed_effect rows[VM_MOCK_ADMIN_ROLE_TIMED_EFFECT_MAX];
    u32 count;
    bool invalid;
} vm_mock_admin_role_timed_effect_list;

static const char *vm_mock_admin_role_timed_effect_label(u8 kind)
{
    switch (kind)
    {
    case VM_NET_MOCK_ROLE_ITEM_EFFECT_EXP_CARD:
        return "修炼丹／经验卡";
    case VM_NET_MOCK_ROLE_ITEM_EFFECT_COMBAT_PILL:
        return "大力丸／神力丸";
    case VM_NET_MOCK_ROLE_ITEM_EFFECT_BATTLE_INSIGHT:
        return "战斗心得";
...
static void vm_mock_admin_role_timed_effect_bonus_text(u8 kind, u8 multiplier,
                                                        char *text,
                                                        size_t textCap)
{
    if (text == NULL || textCap == 0)
        return;
    if (kind == VM_NET_MOCK_ROLE_ITEM_EFFECT_EXP_CARD)
    {
        snprintf(text, textCap, "%u 倍经验", multiplier);
    }
    else if (kind == VM_NET_MOCK_ROLE_ITEM_EFFECT_BATTLE_INSIGHT)
    {
        snprintf(text, textCap, "战斗经验 +%u%%", multiplier);
    }
    else if (kind == VM_NET_MOCK_ROLE_ITEM_EFFECT_EVENT_ATTACK)
    {
        snprintf(text, textCap, "攻击 +%u%%", multiplier);
    }
    else if (kind == VM_NET_MOCK_ROLE_ITEM_EFFECT_EVENT_DEFENSE)
    {
        snprintf(text, textCap, "防御 +%u%%", multiplier);
    }
    else
    {
        snprintf(text, textCap, "攻击、防御 +%u%%", multiplier);
    }
}

static void vm_mock_admin_role_timed_effect_remaining_text(u32 expiresUnix,
                                                            char *text,
                                                            size_t textCap)
{
    u32 now = (u32)time(NULL);
    u32 remaining = expiresUnix > now ? expiresUnix - now : 0;

    if (text == NULL || textCap == 0)
        return;
    if (remaining >= 86400)
    {
        snprintf(text, textCap, "剩余 %u 天 %u 小时", remaining / 86400,
                 (remaining % 86400) / 3600);
    }
    else if (remaining >= 3600)
    {
        snprintf(text, textCap, "剩余 %u 小时 %u 分钟", remaining / 3600,
                 (remaining % 3600) / 60);
    }
    else if (remaining >= 60)
    {
        snprintf(text, textCap, "剩余 %u 分钟", remaining / 60);
    }
    else
    {
        snprintf(text, textCap, "剩余 %u 秒", remaining);
    }
}

static bool vm_mock_admin_role_timed_effect_row(void *contextValue,
                                                unsigned int columnCount,
                                                const char *const *values,
                                                const size_t *lengths)
{
    vm_mock_admin_role_timed_effect_list *list =
        (vm_mock_admin_role_timed_effect_list *)contextValue;
    vm_net_mock_role_item_effect effect;
    u32 kind = 0;
    u32 multiplier = 0;

    if (list == NULL || list->count >= VM_MOCK_ADMIN_ROLE_TIMED_EFFECT_MAX ||
        columnCount != 4 || values == NULL || lengths == NULL ||
        !vm_mock_mysql_parse_u32(values[0], lengths[0], &kind) ||
        !vm_mock_mysql_parse_u32(values[1], lengths[1], &effect.itemId) ||
        !vm_mock_mysql_parse_u32(values[2], lengths[2], &multiplier) ||
        !vm_mock_mysql_parse_u32(values[3], lengths[3], &effect.expiresUnix) ||
        kind == 0 || kind > VM_NET_MOCK_ROLE_ITEM_EFFECT_EVENT_ATTACK_DEFENSE ||
        multiplier > 255)
    {
        if (list != NULL)
            list->invalid = true;
    ...
    effect.kind = (u8)kind;
    effect.multiplier = (u8)multiplier;
    if (!vm_net_mock_role_item_effect_is_valid(&effect))
        return true;
    list->rows[list->count].kind = effect.kind;
    list->rows[list->count].itemId = effect.itemId;
    list->rows[list->count].multiplier = effect.multiplier;
    list->rows[list->count].expiresUnix = effect.expiresUnix;
    list->count++;
    return true;
}

static bool vm_mock_admin_role_timed_effects_load(
    const char *account, u32 roleId, vm_mock_admin_role_timed_effect_list *list)
{
    char accountHex[129];
    char query[768];
    u32 now = (u32)time(NULL);

    if (list == NULL || !vm_mock_admin_account_id_is_valid(account) ||
        roleId == 0 ||
        vm_mysql_hex_encode((const u8 *)account, strlen(account), accountHex,
                            sizeof(accountHex)) == 0 ||
    ...
    memset(list, 0, sizeof(*list));
    snprintf(query, sizeof(query),
             "SELECT effect_kind,item_id,multiplier,expires_unix "
             "FROM account_role_item_effects "
             "WHERE account_id=CAST(X'%s' AS CHAR) AND role_id=%u "
             "AND expires_unix>%u ORDER BY expires_unix ASC,effect_kind ASC",
             accountHex, roleId, now);
    return vm_mysql_query(query, vm_mock_admin_role_timed_effect_row, list) &&
           !list->invalid;
}
...
typedef struct
{
    bool found;
    bool invalid;
} vm_mock_admin_role_timed_effect_match;

static bool vm_mock_admin_role_timed_effect_match_row(
    void *contextValue, unsigned int columnCount, const char *const *values,
    const size_t *lengths)
{
    vm_mock_admin_role_timed_effect_match *match =
        (vm_mock_admin_role_timed_effect_match *)contextValue;
    u32 value = 0;

    if (match == NULL || match->found || columnCount != 1 || values == NULL ||
        lengths == NULL || !vm_mock_mysql_parse_u32(values[0], lengths[0],
                                                     &value) ||
        value != 1)
    {
        if (match != NULL)
...
/* The render form carries the exact durable record snapshot.  Lock that same
 * row before deleting so a late admin click cannot remove an effect which was
 * renewed or replaced by the game in the meantime. */
static bool vm_mock_admin_role_timed_effect_remove(
    const char *account, u32 roleId, u8 kind, u32 itemId, u8 multiplier,
    u32 expiresUnix, const char **errorOut)
{
    char accountHex[129];
    char query[768];
    vm_mock_admin_role_timed_effect_match match;
    ...
    if (errorOut != NULL)
        *errorOut = "时效道具效果删除失败";
    if (!vm_mock_admin_account_id_is_valid(account) || roleId == 0 ||
        kind == 0 || kind > VM_NET_MOCK_ROLE_ITEM_EFFECT_EVENT_ATTACK_DEFENSE ||
        itemId == 0 || expiresUnix == 0 ||
        vm_mysql_hex_encode((const u8 *)account, strlen(account), accountHex,
                            sizeof(accountHex)) == 0)
    {
        if (errorOut != NULL)
    ...
    if (!vm_net_mock_role_prepare_item_effect_schema())
    {
        if (errorOut != NULL)
            *errorOut = "时效道具效果表不可用，请检查数据库";
        return false;
    }
    if (!vm_mysql_exec("START TRANSACTION"))
    {
        if (errorOut != NULL)
            *errorOut = "时效道具效果删除事务启动失败";
        return false;
    }
    transactionStarted = true;
    memset(&match, 0, sizeof(match));
    snprintf(query, sizeof(query),
             "SELECT 1 FROM account_role_item_effects "
             "WHERE account_id=CAST(X'%s' AS CHAR) AND role_id=%u "
             "AND effect_kind=%u AND item_id=%u AND multiplier=%u "
             "AND expires_unix=%u AND expires_unix>%u FOR UPDATE",
             accountHex, roleId, kind, itemId, multiplier, expiresUnix, now);
    if (!vm_mysql_query(query, vm_mock_admin_role_timed_effect_match_row,
                        &match) ||
        match.invalid)
    {
        if (errorOut != NULL)
            *errorOut = "时效道具效果读取失败，请检查数据库";
        goto failed;
    }
    if (!match.found)
    {
        if (errorOut != NULL)
            *errorOut = "该时效道具效果已到期、已变更或已移除，请刷新后重试";
        goto failed;
    }
    snprintf(query, sizeof(query),
             "DELETE FROM account_role_item_effects "
             "WHERE account_id=CAST(X'%s' AS CHAR) AND role_id=%u "
             "AND effect_kind=%u AND item_id=%u AND multiplier=%u "
             "AND expires_unix=%u AND expires_unix>%u",
             accountHex, roleId, kind, itemId, multiplier, expiresUnix, now);
    if (!vm_mysql_exec(query) || !vm_mysql_exec("COMMIT"))
    {
        if (errorOut != NULL)
            *errorOut = "时效道具效果删除失败，请检查数据库";
        goto failed;
    }
    transactionStarted = false;
    printf("[info][mock-admin] timed_item_effect_removed account=%s role=%u kind=%u item=%u multiplier=%u expires=%u\n",
           account, roleId, kind, itemId, multiplier, expiresUnix);
    return true;

failed:
    if (transactionStarted)
        (void)vm_mysql_exec("ROLLBACK");
    printf("[error][mock-admin] timed_item_effect_remove_failed account=%s role=%u kind=%u item=%u error=%s\n",
           account, roleId, kind, itemId, vm_mysql_last_error());
    return false;
}
...
static void vm_mock_admin_render_role_operation_modal(
    ...
    const vm_mock_admin_role_timed_effect_list *timedEffects,
    bool timedEffectsLoaded, const vm_mock_admin_scene_file *sceneFiles,
    u32 sceneCount)
{
    char pickerId[64];
    ...
    vm_mock_admin_text_appendf(
        ...
        "<button type=\"button\" data-role-operation-tab=\"timed-effects\">时效道具</button>"
    ...
    vm_mock_admin_text_appendf(
        ...
        "<section class=\"role-operation-pane\" data-role-operation-pane=\"timed-effects\" hidden><h4>生效中的时效道具</h4><p class=\"muted\">仅显示当前仍生效的效果。移除后，角色后续的经验与战斗属性计算会立即不再使用该项加成；原道具不会返还。</p><div class=\"role-equipment-list role-timed-effect-list\" data-role-timed-effect-list>");
    {
        if (!timedEffectsLoaded)
        {
            vm_mock_admin_text_appendf(
                page,
                "<p class=\"role-equipment-empty\">暂时无法读取时效道具数据，请检查数据库连接。</p>");
        }
        else if (timedEffects == NULL || timedEffects->count == 0)
        {
            vm_mock_admin_text_appendf(
                page,
                "<p class=\"role-equipment-empty\">该角色当前没有生效中的时效道具。</p>");
        }
        else
        {
            for (u32 timedEffectIndex = 0;
                 timedEffectIndex < timedEffects->count; ++timedEffectIndex)
            {
                const vm_mock_admin_role_timed_effect *effect =
                    &timedEffects->rows[timedEffectIndex];
                const vm_net_mock_shop_catalog_item *catalogItem =
                    vm_net_mock_find_shop_catalog_item(effect->itemId);
                char itemNameUtf8[128];
                char bonusText[64];
                char remainingText[64];

                memset(itemNameUtf8, 0, sizeof(itemNameUtf8));
                memset(bonusText, 0, sizeof(bonusText));
                memset(remainingText, 0, sizeof(remainingText));
                if (catalogItem != NULL)
                {
                ...
                vm_mock_admin_role_timed_effect_bonus_text(
                    effect->kind, effect->multiplier, bonusText,
                    sizeof(bonusText));
                vm_mock_admin_role_timed_effect_remaining_text(
                    effect->expiresUnix, remainingText, sizeof(remainingText));
                vm_mock_admin_text_appendf(
                    page,
                    "<article class=\"role-equipment-row\"><div class=\"role-equipment-summary\"><span class=\"role-equipment-slot\">%s</span><strong>",
                    vm_mock_admin_role_timed_effect_label(effect->kind));
                if (itemNameUtf8[0] != 0)
                    vm_mock_admin_text_append_html(page, itemNameUtf8);
                else
                    vm_mock_admin_text_appendf(page, "未知物品 #%u", effect->itemId);
                vm_mock_admin_text_appendf(
                    page,
                    "</strong><small>物品 ID %u · %s · %s</small></div><form class=\"role-equipment-form\" method=\"post\" action=\"/action\" data-admin-confirm=\"确定移除该时效道具效果？移除后角色立即不再获得该项加成，原道具不会返还。\"><input type=\"hidden\" name=\"action\" value=\"remove-role-timed-item-effect\"><input type=\"hidden\" name=\"account\" value=\"",
                    effect->itemId, bonusText, remainingText);
                vm_mock_admin_text_append_html(page, account);
                vm_mock_admin_text_appendf(
                    page,
                    "\"><input type=\"hidden\" name=\"role\" value=\"%u\"><input type=\"hidden\" name=\"effect_kind\" value=\"%u\"><input type=\"hidden\" name=\"item\" value=\"%u\"><input type=\"hidden\" name=\"multiplier\" value=\"%u\"><input type=\"hidden\" name=\"expires_unix\" value=\"%u\"><button class=\"reset-position\" type=\"submit\">移除效果</button></form></article>",
                    role->roleId, effect->kind, effect->itemId,
                    effect->multiplier, effect->expiresUnix);
            }
        }
    }
    vm_mock_admin_text_appendf(
    ...
    vm_mock_admin_text_appendf(
        page,
        "\"><input type=\"hidden\" name=\"role\" value=\"%u\"><label><span>目标场景</span>",
        role->roleId);
    vm_mock_admin_render_role_reset_scene_picker_field(page, sceneFiles,
                                                        sceneCount);
    vm_mock_admin_text_appendf(
        page,
        "</label><div class=\"role-operation-actions\"><button class=\"reset-position\" type=\"submit\">重置到指定场景</button></div></form></section>"
        "</div></div></section></div>");
}
    ...
    vm_mock_admin_text *page, const char *name, u8 current, u8 slot)
{
    ...
    vm_mock_admin_text_appendf(
        page, "<select name=\"%s\" data-task-requirement-type=\"%u\">",
        name, slot);
...
enum
{
    VM_MOCK_ADMIN_NEW_TASK_ID_MIN = 2000
};

/* New task IDs live outside the built-in task range.  Search the complete
 * catalog instead of deriving an ID from its last row: imported task.dsh
 * rows and MySQL overrides are not guaranteed to be ordered or contiguous. */
static bool vm_mock_admin_find_first_unused_task_id(
    const vm_net_mock_task_definition *tasks, u32 taskCount, u32 *taskIdOut)
{
    if (tasks == NULL || taskIdOut == NULL)
        return false;
    for (u32 candidate = VM_MOCK_ADMIN_NEW_TASK_ID_MIN; candidate != 0;
         ++candidate)
    {
        bool used = candidate == VM_NET_MOCK_TEST_TASK_ID;

        for (u32 i = 0; !used && i < taskCount; ++i)
            used = tasks[i].taskId == candidate;
        if (!used)
        {
            *taskIdOut = candidate;
            return true;
                                           ...
                                           const char *query)
{
    ...
    char goalUtf8[384];
    char rewardUtf8[128];
    char requirementScene1Utf8[192];
    char requirementScene2Utf8[192];
    char offerUtf8[768];
    char activeUtf8[768];
    ...
    memset(requirementScene1Utf8, 0, sizeof(requirementScene1Utf8));
    memset(requirementScene2Utf8, 0, sizeof(requirementScene2Utf8));
    ...
    if (createNew)
    {
        ...
        else
        {
            ...
            snprintf(message, sizeof(message), "没有可分配的任务 ID");
        }
    }
    if (found)
    {
        ...
        vm_net_mock_gbk_label_to_utf8(edit.requirementScene1,
                                      requirementScene1Utf8,
                                      sizeof(requirementScene1Utf8));
        vm_net_mock_gbk_label_to_utf8(edit.requirementScene2,
                                      requirementScene2Utf8,
                                      sizeof(requirementScene2Utf8));
    ...
    vm_mock_admin_text_appendf(&page, "<label class=\"field\"><span>任务 ID</span><input type=\"number\" name=\"task_id\" min=\"%u\" max=\"4294967295\" value=\"%u\" readonly required></label>", createNew ? VM_MOCK_ADMIN_NEW_TASK_ID_MIN : 1u, edit.taskId);
    ...
    vm_mock_admin_text_appendf(
        &page,
        "</div><div class=\"group\"><h2>任务目标</h2><p class=\"hint\">默认无目标。新增后可配置收集物品或击败怪物；击败怪物可额外限定 SCE 场景。物流交付任务请保持目标为空，并在下方设置接取给予物品及交付 NPC。当前客户端协议最多支持 2 个目标。</p><div class=\"task-target-list\" id=\"task-requirement-list\">");
    vm_mock_admin_render_task_requirement_row(
        &page, 1, edit.requirementType1, edit.requirementId1,
        edit.requirementCount1, requirementScene1Utf8);
    vm_mock_admin_render_task_requirement_row(
        &page, 2, edit.requirementType2, edit.requirementId2,
        edit.requirementCount2, requirementScene2Utf8);
    vm_mock_admin_text_appendf(
        &page,
        "<p class=\"task-target-empty\" data-task-requirement-empty hidden>暂未配置任务目标。</p></div><div class=\"actions task-target-actions\"><button class=\"secondary\" type=\"button\" id=\"task-requirement-add\" data-task-requirement-add>＋ 新增任务目标</button></div><label class=\"field\" style=\"margin-top:12px\"><span>目标说明（最多95字节）</span><textarea name=\"goal\" maxlength=\"95\">");
    vm_mock_admin_text_append_html(&page, goalUtf8);
    vm_mock_admin_text_appendf(&page, "</textarea></label></div>");
    vm_mock_admin_text_appendf(&page, "<div class=\"group\"><h2>给予物品与奖励</h2><div class=\"fields\">");
    vm_mock_admin_render_item_picker_field(&page, "task-given-item", "given_item_id", "接取给予物品（物流货物）", edit.givenItemId, false);
    ...
    bool required, bool allowNone, bool disabled)
{
    ...
    vm_mock_admin_text_appendf(
        page,
        "<div class=\"monster-picker-field\"><select class=\"monster-resource-select\" name=\"");
    vm_mock_admin_text_append_html(page, fieldName);
    vm_mock_admin_text_appendf(
        page, "\"%s%s%s hidden>", required ? " required" : "",
        allowNone ? " data-monster-picker-allow-none=\"1\"" : "",
        disabled ? " disabled" : "");
    if (allowNone)
    {
        vm_mock_admin_text_appendf(
            page, "<option value=\"0\"%s>不额外刷新</option>",
            monsterId == 0 ? " selected" : "");
    }
    else
    {
        vm_mock_admin_text_appendf(page,
    ...
    {
        vm_mock_admin_text_appendf(
            page, "<option value=\"%u\" selected>#%u</option>",
            monsterId, monsterId);
    }
    vm_mock_admin_text_appendf(
        page,
        "</select><button class=\"item-picker-trigger monster-picker-trigger\" type=\"button\" data-monster-picker-open aria-haspopup=\"dialog\" aria-controls=\"monster-picker-modal\"><span data-monster-picker-label>");
    if (monsterId != 0)
        vm_mock_admin_text_appendf(page, "已选怪物 #%u", monsterId);
    else if (allowNone)
        vm_mock_admin_text_appendf(page, "不额外刷新");
    else
        vm_mock_admin_text_appendf(page, "请选择怪物");
    vm_mock_admin_text_appendf(
        page, "</span><small>搜索怪物</small></button></div>");
}

static void vm_mock_admin_render_monster_picker_modal(vm_mock_admin_text *page)
{
    vm_net_mock_monster_admin_row
        monsters[VM_NET_MOCK_MONSTER_CATALOG_MAX];
    u32 monsterCount = 0;
    ...
    monsterCount = vm_net_mock_monster_admin_list(
        monsters, VM_NET_MOCK_MONSTER_CATALOG_MAX);
    if (monsterCount > VM_NET_MOCK_MONSTER_CATALOG_MAX)
        monsterCount = VM_NET_MOCK_MONSTER_CATALOG_MAX;

    vm_mock_admin_text_appendf(
        page,
        "<div class=\"item-modal actor-modal monster-picker-modal\" id=\"monster-picker-modal\" role=\"dialog\" aria-modal=\"true\" aria-labelledby=\"monster-picker-title\" hidden><section class=\"item-picker-panel actor-picker-panel monster-picker-panel\"><div class=\"item-picker-head actor-picker-head\"><div><h3 id=\"monster-picker-title\">选择怪物</h3><p>可按怪物 ID、名称、等级或类型搜索；选择后会更新当前表单。</p></div><button class=\"item-picker-close actor-picker-close\" id=\"monster-picker-close\" type=\"button\" aria-label=\"关闭怪物选择\">×</button></div><div class=\"item-picker-tools actor-picker-tools\"><label><span>搜索怪物</span><input id=\"monster-picker-search\" type=\"search\" placeholder=\"输入怪物 ID、名称、等级或类型\" autocomplete=\"off\"></label></div><div class=\"item-result-bar actor-result-bar\"><span id=\"monster-picker-result-count\"></span></div><p class=\"item-picker-error\" id=\"monster-picker-error\"></p><div class=\"item-picker-list actor-picker-list monster-picker-list\" id=\"monster-picker-list\"></div><p class=\"item-picker-empty actor-picker-empty\" id=\"monster-picker-empty\" hidden>没有符合条件的怪物。</p><select id=\"monster-picker-options\" hidden>");
    for (u32 i = 0; i < monsterCount; ++i)
    {
        char nameUtf8[128];

        memset(nameUtf8, 0, sizeof(nameUtf8));
        vm_net_mock_gbk_label_to_utf8(monsters[i].displayName, nameUtf8,
                                      sizeof(nameUtf8));
        vm_mock_admin_text_appendf(page, "<option value=\"%u\">#%u · ",
                                   monsters[i].enemyId, monsters[i].enemyId);
        vm_mock_admin_text_append_html(
            page, nameUtf8[0] != 0 ? nameUtf8 : "未命名怪物");
        vm_mock_admin_text_appendf(
            page, " · Lv.%u · %s</option>", monsters[i].level,
            vm_mock_admin_monster_family_name(monsters[i].family));
    }
    vm_mock_admin_text_appendf(page, "</select></section></div>");
}
...
typedef struct
{
    vm_mock_admin_risk_ip_row rows[VM_MOCK_ADMIN_RISK_IP_PAGE_SIZE];
    u32 count;
    bool invalid;
...
static bool vm_mock_admin_risk_ip_row_callback(
    ...
    const char *const *values, const size_t *lengths)
{
    vm_mock_admin_risk_ip_page *page =
        (vm_mock_admin_risk_ip_page *)contextValue;
    ...
    row = &page->rows[page->count];
    if (!vm_mock_mysql_copy_text(row->address, sizeof(row->address), values[0],
                                 lengths[0]) ||
...
static bool vm_mock_admin_risk_ip_build_query(char *sql, size_t sqlCap,
                                              u32 offset, u32 limit)
{
    int written = 0;
...
static bool vm_mock_admin_risk_ip_query(u32 offset, u32 limit,
                                        vm_mock_admin_risk_ip_page *page)
{
    char sql[512];

    if (page == NULL || !vm_mock_service_login_ip_schema_ensure() ||
        !vm_mock_admin_risk_ip_build_query(sql, sizeof(sql), offset, limit))
    {
        return false;
    }
    memset(page, 0, sizeof(*page));
    return vm_mysql_query(sql, vm_mock_admin_risk_ip_row_callback, page) &&
           !page->invalid;
}

static bool vm_mock_admin_risk_ip_query_count(u32 *countOut)
{
    static const char sql[] =
        "SELECT COUNT(*) FROM server_login_ip_blocks WHERE blocked=1";
    vm_mock_admin_count count;
                                           ...
                                           const char *query)
{
    ...
    char message[256];
    char pageText[16];
    char directoryText[16];
    u32 totalCount = 0;
    u32 pageNumber = 1;
    u32 pageCount = 1;
    u32 offset = 0;
    u32 pageSize = VM_MOCK_ADMIN_RISK_AUDIT_PAGE_SIZE;
    vm_mock_admin_risk_directory directory =
        VM_MOCK_ADMIN_RISK_DIRECTORY_ROLES;
    bool queryOk = false;
    ...
    memset(message, 0, sizeof(message));
    memset(pageText, 0, sizeof(pageText));
    memset(directoryText, 0, sizeof(directoryText));
    (void)vm_mock_admin_form_value(query, "status", status, sizeof(status));
    (void)vm_mock_admin_form_value(query, "message", message, sizeof(message));
    (void)vm_mock_admin_form_value(query, "page", pageText, sizeof(pageText));
    (void)vm_mock_admin_form_value(query, "risk_kind", directoryText,
                                   sizeof(directoryText));
    if (strcmp(directoryText, "ips") == 0)
    {
    ...
    {
    vm_mock_admin_text_appendf(&page,
        "<section class=\"card\"><h2>风险角色审计列表</h2>");
    if (!queryOk)
    {
        vm_mock_admin_text_appendf(&page,
    ...
    else
    {
        ...
        if (pageNumber > 1)
            vm_mock_admin_text_appendf(
                &page, "<a href=\"/?tab=risk&amp;risk_kind=roles&amp;page=%u\">上一页</a>",
                pageNumber - 1);
        if (pageNumber < pageCount)
            vm_mock_admin_text_appendf(
                &page, "<a href=\"/?tab=risk&amp;risk_kind=roles&amp;page=%u\">下一页</a>",
                pageNumber + 1);
        vm_mock_admin_text_appendf(&page,
    ...
    else if (directory == VM_MOCK_ADMIN_RISK_DIRECTORY_IPS)
    {
        vm_mock_admin_text_appendf(
            &page,
            "<section class=\"card\"><h2>风险 IP 列表 <span class=\"badge\">已封锁</span></h2>");
        if (!queryOk)
        {
            vm_mock_admin_text_appendf(
                &page,
                "<p class=\"muted\">无法读取已封锁 IP：%s</p></section></main></body></html>",
                vm_mysql_last_error());
        }
        else if (ipPage.count == 0)
        {
            vm_mock_admin_text_appendf(
                &page,
                "<p class=\"muted\">暂无已封锁的来源 IP。</p></section></main></body></html>");
        }
        else
        {
            vm_mock_admin_text_appendf(
            ...
            for (u32 i = 0; i < ipPage.count; ++i)
            {
                ...
                vm_mock_admin_text_appendf(
                    &page,
                    "\"><input type=\"hidden\" name=\"page\" value=\"%u\"><button type=\"submit\">清除封锁</button></form></td></tr>",
                    pageNumber);
            }
            vm_mock_admin_text_appendf(
                &page,
            ...
            if (pageNumber > 1)
                vm_mock_admin_text_appendf(
                    &page,
                    "<a href=\"/?tab=risk&amp;risk_kind=ips&amp;page=%u\">上一页</a>",
                    pageNumber - 1);
            if (pageNumber < pageCount)
                vm_mock_admin_text_appendf(
                    &page,
                    "<a href=\"/?tab=risk&amp;risk_kind=ips&amp;page=%u\">下一页</a>",
                    pageNumber + 1);
            vm_mock_admin_text_appendf(
    ...
    {
        snprintf(response, responseCap,
                 "<!doctype html><meta charset=\"utf-8\"><p>风险管理页面超过大小限制。</p>");
    }
}

/* Account and role writes are committed by their owning service functions.
 * This append-only audit trail is deliberately independent of those parent
 * rows, so a later role migration or deletion cannot erase the history of a
 * completed administrator action. */
typedef struct
{
    uint64_t logId;
    char operatorAccountId[64];
    ...
    char detail[256];
    char createdAt[40];
} vm_mock_admin_operation_log_row;

typedef struct
{
    vm_mock_admin_operation_log_row rows[VM_MOCK_ADMIN_OPERATION_LOG_PAGE_SIZE];
    u32 count;
    bool invalid;
} vm_mock_admin_operation_log_page;

typedef struct
{
    const char *actionCode;
    const char *label;
} vm_mock_admin_operation_log_action;
...
static const vm_mock_admin_operation_log_action
    ...
    {"remove-role-timed-item-effect", "移除时效道具效果"},
    ...
    {"recycle-equipment", "装备回收"},
    {"spend-wcoin-shop", "游戏内商城消费 W 币"},
    {"spend-wcoin-instance", "付费副本消费 W 币"}
};

enum
{
    VM_MOCK_ADMIN_OPERATION_LOG_ACTION_COUNT =
        sizeof(g_vm_mock_admin_operation_log_actions) /
        sizeof(g_vm_mock_admin_operation_log_actions[0])
};
...
static bool vm_mock_admin_operation_log_schema_ensure(void)
{
    ...
    if (!vm_mysql_exec(
            ...
            ") ENGINE=InnoDB"))
    {
        printf("[error][mock-admin] operation_log_schema_prepare error=%s\n",
               vm_mysql_last_error());
        return false;
    }
...
static bool vm_mock_admin_operation_log_row_callback(
    void *contextValue, unsigned int columnCount,
    const char *const *values, const size_t *lengths)
{
    vm_mock_admin_operation_log_page *page =
        (vm_mock_admin_operation_log_page *)contextValue;
...
static u32 vm_mock_admin_operation_log_action_filter_from_query(
    const char *query)
{
    char selected[VM_MOCK_ADMIN_OPERATION_LOG_ACTION_COUNT][33];
    u32 selectedCount = 0;
    u32 filter = 0;

    memset(selected, 0, sizeof(selected));
    if (!vm_mock_admin_form_values(query, "type", (char *)selected,
                                   sizeof(selected[0]),
                                   VM_MOCK_ADMIN_OPERATION_LOG_ACTION_COUNT,
                                   &selectedCount))
    {
    ...
    {
        for (u32 j = 0; j < VM_MOCK_ADMIN_OPERATION_LOG_ACTION_COUNT; ++j)
        {
            if (strcmp(selected[i],
                       g_vm_mock_admin_operation_log_actions[j].actionCode) == 0)
            {
                filter |= 1u << j;
                break;
            }
...
static bool vm_mock_admin_operation_log_build_filter(const char *accountId,
                                                      u32 actionFilter,
                                                      char *filter,
                                                      size_t filterCap)
{
    ...
    if (accountId != NULL && accountId[0] != 0)
    {
        ...
        if (strlen(accountId) > 63 ||
            vm_mysql_hex_encode((const u8 *)accountId, strlen(accountId),
                                accountHex, sizeof(accountHex)) == 0)
        {
            return false;
        }
        written = snprintf(filter, filterCap,
                           "WHERE target_account_id=CAST(X'%s' AS CHAR)",
                           accountHex);
        if (written < 0 || (size_t)written >= filterCap)
            return false;
        used = (size_t)written;
        haveCondition = true;
    }
    if (actionFilter != 0)
    {
        int written = snprintf(filter + used, filterCap - used, "%s(",
                               haveCondition ? " AND " : "WHERE ");

        if (written < 0 || (size_t)written >= filterCap - used)
            return false;
        used += (size_t)written;
        for (u32 i = 0; i < VM_MOCK_ADMIN_OPERATION_LOG_ACTION_COUNT; ++i)
        {
            if ((actionFilter & (1u << i)) == 0)
                continue;
            written = snprintf(filter + used, filterCap - used, "%saction_code='%s'",
                               haveAction ? " OR " : "",
                               g_vm_mock_admin_operation_log_actions[i].actionCode);
            if (written < 0 || (size_t)written >= filterCap - used)
                return false;
            used += (size_t)written;
            haveAction = true;
        }
        if (!haveAction || used + 1 >= filterCap)
            return false;
        filter[used++] = ')';
        filter[used] = 0;
    }
...
static void vm_mock_admin_operation_log_append_action_filter_query(
    u32 actionFilter, char *out, size_t outCap)
{
    size_t used = 0;
    ...
    for (u32 i = 0; i < VM_MOCK_ADMIN_OPERATION_LOG_ACTION_COUNT; ++i)
    {
        ...
        if ((actionFilter & (1u << i)) == 0)
            continue;
        written = snprintf(out + used, outCap - used, "&amp;type=%s",
                           g_vm_mock_admin_operation_log_actions[i].actionCode);
...
static bool vm_mock_admin_operation_log_query(
    const char *accountId, u32 actionFilter, u32 offset, u32 limit,
    vm_mock_admin_operation_log_page *page)
{
    char filter[768];
    char sql[1024];
    ...
    if (page == NULL || limit == 0 ||
        !vm_mock_admin_operation_log_schema_ensure() ||
        !vm_mock_admin_operation_log_build_filter(accountId, actionFilter,
    ...
    snprintf(sql, sizeof(sql),
             ...
             "LIMIT %u,%u",
             filter, offset, limit);
    return vm_mysql_query(sql, vm_mock_admin_operation_log_row_callback, page) &&
           !page->invalid;
}

static bool vm_mock_admin_operation_log_query_count(const char *accountId,
                                                    u32 actionFilter,
                                                    u32 *countOut)
{
    char filter[768];
    ...
    if (countOut != NULL)
        *countOut = 0;
    if (!vm_mock_admin_operation_log_schema_ensure() ||
        !vm_mock_admin_operation_log_build_filter(accountId, actionFilter,
                                                  filter, sizeof(filter)))
        return false;
    memset(&count, 0, sizeof(count));
    snprintf(sql, sizeof(sql),
             "SELECT COUNT(*) FROM server_admin_operation_logs %s", filter);
    if (!vm_mysql_query(sql, vm_mock_admin_count_row, &count) ||
        count.invalid || !count.found)
    {
        return false;
...
static const char *vm_mock_admin_operation_log_action_label(
    const char *actionCode)
{
    if (actionCode == NULL)
        return "未知操作";
    for (u32 i = 0; i < VM_MOCK_ADMIN_OPERATION_LOG_ACTION_COUNT; ++i)
    {
        if (strcmp(actionCode,
                   g_vm_mock_admin_operation_log_actions[i].actionCode) == 0)
        {
            return g_vm_mock_admin_operation_log_actions[i].label;
        }
    }
    return actionCode;
}
...
static bool vm_mock_admin_operation_log_is_wcoin_debit(const char *actionCode)
{
    return actionCode != NULL &&
           (strcmp(actionCode, "remove-wcoin") == 0 ||
            strncmp(actionCode, "spend-wcoin-", 12) == 0);
}
...
{
    vm_mock_admin_text page;
    vm_mock_admin_operation_log_page logPage;
    char accountFilter[64];
    char encodedAccount[128];
    char encodedActionFilter[512];
    char pageText[16];
    u32 totalCount = 0;
    u32 pageNumber = 1;
    u32 pageCount = 1;
    ...
    memset(accountFilter, 0, sizeof(accountFilter));
    memset(encodedAccount, 0, sizeof(encodedAccount));
    memset(encodedActionFilter, 0, sizeof(encodedActionFilter));
    memset(pageText, 0, sizeof(pageText));
    (void)vm_mock_admin_form_value(query, "account", accountFilter,
                                   sizeof(accountFilter));
    actionFilter = vm_mock_admin_operation_log_action_filter_from_query(query);
    (void)vm_mock_admin_form_value(query, "page", pageText, sizeof(pageText));
    if (pageText[0] != 0 &&
        (!vm_net_mock_parse_u32_strict(pageText, &pageNumber) || pageNumber == 0))
    {
    ...
    queryOk = vm_mock_admin_operation_log_query_count(accountFilter,
                                                       actionFilter, &totalCount);
    if (queryOk)
    {
        if (totalCount != 0)
            pageCount = (totalCount + VM_MOCK_ADMIN_OPERATION_LOG_PAGE_SIZE - 1) /
                        VM_MOCK_ADMIN_OPERATION_LOG_PAGE_SIZE;
        if (pageNumber > pageCount)
            pageNumber = pageCount;
        offset = (pageNumber - 1) * VM_MOCK_ADMIN_OPERATION_LOG_PAGE_SIZE;
        queryOk = vm_mock_admin_operation_log_query(
            accountFilter, actionFilter, offset,
            VM_MOCK_ADMIN_OPERATION_LOG_PAGE_SIZE, &logPage);
    }
    vm_mock_admin_url_encode(accountFilter, encodedAccount,
                             sizeof(encodedAccount));
    vm_mock_admin_operation_log_append_action_filter_query(
        actionFilter, encodedActionFilter, sizeof(encodedActionFilter));
    ...
    vm_mock_admin_text_appendf(
        &page,
        "\" placeholder=\"输入精确账号名\"></label><fieldset class=\"type-filter\"><legend>日志类型（可多选）</legend><div class=\"type-options\">");
    for (u32 i = 0; i < VM_MOCK_ADMIN_OPERATION_LOG_ACTION_COUNT; ++i)
    {
        vm_mock_admin_text_appendf(
            &page, "<label><input type=\"checkbox\" name=\"type\" value=\"%s\"%s>",
            g_vm_mock_admin_operation_log_actions[i].actionCode,
            (actionFilter & (1u << i)) != 0 ? " checked" : "");
        vm_mock_admin_text_append_html(
            &page, g_vm_mock_admin_operation_log_actions[i].label);
        vm_mock_admin_text_appendf(&page, "</label>");
    }
    vm_mock_admin_text_appendf(
        &page,
    ...
    if (!queryOk)
    {
        vm_mock_admin_text_appendf(
            &page,
            "<p class=\"muted\">无法读取操作日志：%s</p></section></main></body></html>",
            vm_mysql_last_error());
    }
    else if (logPage.count == 0)
    {
        vm_mock_admin_text_appendf(
            &page,
            "<p class=\"muted\">暂无符合条件的操作记录。</p></section></main></body></html>");
    }
    else
    {
        vm_mock_admin_text_appendf(
        ...
        for (u32 i = 0; i < logPage.count; ++i)
        {
            ...
            vm_mock_admin_text_appendf(&page, "</td><td class=\"mono\">");
            if (row->roleId != 0)
                vm_mock_admin_text_appendf(&page, "ID %u", row->roleId);
            else
                vm_mock_admin_text_appendf(&page, "账号级");
            vm_mock_admin_text_appendf(&page, "</td><td class=\"mono\">");
            if (row->itemId != 0 && row->changeAmount != 0)
                vm_mock_admin_text_appendf(&page, "物品 %u × %u；W币 -%u",
                                           row->itemId, row->itemCount,
                                           row->changeAmount);
            else if (row->itemId != 0)
                vm_mock_admin_text_appendf(&page, "物品 %u × %u", row->itemId,
                                           row->itemCount);
            else if (row->changeAmount != 0)
                vm_mock_admin_text_appendf(
                    &page, "%s%u",
                    vm_mock_admin_operation_log_is_wcoin_debit(row->actionCode)
                        ? "W币 -" : "",
                    row->changeAmount);
            else
                vm_mock_admin_text_appendf(&page, "—");
            vm_mock_admin_text_appendf(&page, "</td><td>");
            vm_mock_admin_text_append_html(&page, row->detail);
            vm_mock_admin_text_appendf(&page, "</td></tr>");
        }
        vm_mock_admin_text_appendf(
            &page,
        ...
        if (pageNumber > 1)
            vm_mock_admin_text_appendf(
                &page,
                "<a href=\"/?tab=operations&amp;account=%s%s&amp;page=%u\">上一页</a>",
                encodedAccount, encodedActionFilter, pageNumber - 1);
        if (pageNumber < pageCount)
            vm_mock_admin_text_appendf(
                &page,
                "<a href=\"/?tab=operations&amp;account=%s%s&amp;page=%u\">下一页</a>",
                encodedAccount, encodedActionFilter, pageNumber + 1);
        vm_mock_admin_text_appendf(
            &page, "</div></div></section></main></body></html>");
    }
    ...
    {
        snprintf(response, responseCap,
                 "<!doctype html><meta charset=\"utf-8\"><p>操作日志页面超过大小限制。</p>");
    }
}
    ...
    char *response, size_t responseCap, const char *query)
{
    ...
    vm_mock_admin_text_append_html(&page, selectedSceneUtf8);
    vm_mock_admin_text_appendf(&page,
        "\"><div class=\"fields battle-monster-fields\"><label class=\"field battle-monster-wide\"><span>复用怪物 ID（可选，可另设坐标）</span>");
    vm_mock_admin_render_monster_picker_field(
        &page, "monster_id", 0, false, false, false);
    vm_mock_admin_text_appendf(
        &page,
        "</label><label class=\"field battle-monster-wide\"><span>参考怪物（未复用时生成新怪物 ID）</span>");
    vm_mock_admin_render_monster_picker_field(
        &page, "source_monster_id", 0, false, false, false);
    vm_mock_admin_text_appendf(
        &page,
    ...
    vm_mock_admin_text_appendf(&page,
        "</label><label class=\"field\"><span>X</span><input name=\"pos_x\" type=\"number\" min=\"17\" max=\"65519\" required></label><label class=\"field\"><span>Y</span><input name=\"pos_y\" type=\"number\" min=\"17\" max=\"65519\" required></label><label class=\"field\"><span>数量</span><input name=\"quantity\" type=\"number\" min=\"1\" max=\"5\" value=\"1\" required></label><label class=\"field\"><span>视觉提示</span><select name=\"visual_hint\"><option value=\"5\">5 · 普通</option><option value=\"6\">6 · 强敌</option></select></label><label class=\"field\"><span>状态</span><select name=\"enabled\"><option value=\"1\">启用</option><option value=\"0\">停用</option></select></label><div class=\"actions battle-monster-actions\"><button type=\"submit\">保存草稿</button></div></div></form></article>");
    for (u32 i = 0; i < rowCount; ++i)
    {
        char nameUtf8[128];
        ...
        vm_mock_admin_text_append_html(&page, selectedSceneUtf8);
        vm_mock_admin_text_appendf(&page,
        "\"><div class=\"fields battle-monster-fields\"><label class=\"field battle-monster-wide\"><span>怪物 ID（可修改为已有怪物）</span>");
        vm_mock_admin_render_monster_picker_field(
    ...
    vm_mock_admin_render_scene_battle_monster_actor_select(
        ...
        vm_mock_admin_text_appendf(&page,
            "</label><label class=\"field\"><span>X</span><input name=\"pos_x\" type=\"number\" min=\"17\" max=\"65519\" value=\"%u\" required></label><label class=\"field\"><span>Y</span><input name=\"pos_y\" type=\"number\" min=\"17\" max=\"65519\" value=\"%u\" required></label><label class=\"field\"><span>数量</span><input name=\"quantity\" type=\"number\" min=\"1\" max=\"5\" value=\"%u\" required></label><label class=\"field\"><span>视觉提示</span><select name=\"visual_hint\"><option value=\"5\"%s>5 · 普通</option><option value=\"6\"%s>6 · 强敌</option></select></label><label class=\"field\"><span>状态</span><select name=\"enabled\"><option value=\"1\"%s>启用</option><option value=\"0\"%s>停用</option></select></label><div class=\"actions\"><button type=\"submit\">保存草稿</button></div></div></form><form class=\"actions\" method=\"post\" action=\"/action\"><input type=\"hidden\" name=\"action\" value=\"delete-scene-battle-monster\"><input type=\"hidden\" name=\"scene\" value=\"",
            rows[i].x, rows[i].y, rows[i].quantity,
            rows[i].visualHint == 5 ? " selected" : "",
    ...
    vm_mock_admin_text_appendf(&page,
        "</div><p class=\"foot\">怪物 ID 决定怪物属性、掉落和任务击败条件；同一 ID 可在不同坐标复用。每次保存“新增下一条”都会保留已有草稿，部署时会同时生成全部启用项。数量会以所填坐标为中心展开为最多 5 个独立节点。本体 Actor 必须来自原生 kind-3 战斗节点，负责碰撞与战斗；退场火团只能填写在 field18。普通 NPC（含任务发布者）不属于此配置层。</p>");
                                                      ...
                                                      const char *query)
{
    ...
    vm_mock_admin_text_appendf(&page,
        "<!doctype html><html lang=\"zh-CN\"><head><meta charset=\"utf-8\">"
        "<meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">"
        "<title>江湖OL 游戏内容管理 · Actor 资源</title><style>"
        ...
        "</style><script src=\"/admin.js\" defer></script></head><body><main class=\"wrap\"><header><div><h1>江湖OL 后台管理</h1><p class=\"sub\">游戏内容管理 · 编辑服务端权威 .actor 结构；保存后会用原生内容更新让客户端重新下载。</p></div><form method=\"post\" action=\"/logout\"><button class=\"logout\" type=\"submit\">退出登录</button></form></header>"
        "<nav class=\"tabs\"><a class=\"tab\" href=\"/?tab=accounts\">账号管理</a><a class=\"tab on\" href=\"/?tab=content\">游戏内容管理</a><a class=\"tab\" href=\"/?tab=tasks\">任务管理</a><a class=\"tab\" href=\"/?tab=monsters\">怪物管理</a><a class=\"tab\" href=\"/?tab=scene-monsters\">场景战斗怪</a><a class=\"tab\" href=\"/?tab=shop\">商品管理</a><a class=\"tab\" href=\"/?tab=chests\">宝箱管理</a><a class=\"tab\" href=\"/?tab=updates\">游戏内容更新管理</a><a class=\"tab\" href=\"/?tab=servers\">服务器列表</a><a class=\"tab\" href=\"/?tab=risk\">风险管理</a></nav><div class=\"layout\"><aside class=\"card\"><h2>文件格式</h2><nav style=\"display:flex;gap:6px;flex-wrap:wrap;margin:10px 0 16px\"><a style=\"padding:7px 9px;border:1px solid #e4e7ec;border-radius:7px;color:#475467;background:#fff;text-decoration:none\" href=\"/?tab=content&amp;content_kind=sce\">SCE 场景</a><a style=\"padding:7px 9px;border:1px solid #e4e7ec;border-radius:7px;color:#475467;background:#fff;text-decoration:none\" href=\"/?tab=content&amp;content_kind=gif\">GIF 图片</a><a style=\"padding:7px 9px;border:1px solid #e4e7ec;border-radius:7px;color:#475467;background:#fff;text-decoration:none\" href=\"/?tab=content&amp;content_kind=dsh\">DSH 数据</a><a style=\"padding:7px 9px;border:1px solid #175cd3;border-radius:7px;color:#fff;background:#175cd3;text-decoration:none\" href=\"/?tab=content&amp;content_kind=actor\">Actor 资源</a></nav><h2>已有 Actor（%u）</h2><div class=\"actor-list\" data-admin-list><a class=\"new-link%s\" data-admin-select%s href=\"/?tab=content&amp;content_kind=actor&amp;new=1\">新建 Actor 资源</a>",
    ...
    for (u32 i = 0; i < fileCount; ++i)
    {
        ...
        vm_mock_admin_text_appendf(&page,
            "<a class=\"actor-link%s\" data-admin-select%s href=\"/?tab=content&amp;content_kind=actor&amp;actor=%s\"><span>",
            selectedActor ? " on" : "",
    ...
    else
    {
        ...
        {
            vm_mock_admin_text_appendf(&page,
                "<form class=\"template\" method=\"get\" action=\"/\"><input type=\"hidden\" name=\"tab\" value=\"content\"><input type=\"hidden\" name=\"content_kind\" value=\"actor\"><input type=\"hidden\" name=\"new\" value=\"1\"><label><span>新建模板</span><select name=\"template\">");
            for (u32 i = 0; i < fileCount; ++i)
            {
                vm_mock_admin_text_appendf(&page, "<option value=\"");
                vm_mock_admin_text_append_html(&page, files[i].name);
                                      ...
                                      const char *query)
{
    ...
    if (strcmp(tab, "content") == 0)
    {
        ...
        {
            vm_mock_admin_render_actor_resource_page(response, responseCap,
                                                     query);
            return;
        }
    ...
    if (!vm_mock_admin_account_query_count(accountSearch, &accountTotal))
        accountTotal = 0;
    onlinePlayerTotal = vm_mock_admin_online_player_count();
    ...
    vm_mock_admin_text_appendf(&page,
        ...
        ".online-player-stat{display:grid;grid-template-columns:auto 1fr;align-items:center;column-gap:9px;row-gap:1px;margin:0 0 12px;padding:10px 11px;border:1px solid #b2ddff;border-radius:8px;background:#eff8ff;color:#175cd3}.online-player-stat strong{grid-row:1/3;font-size:24px;line-height:1;font-variant-numeric:tabular-nums}.online-player-stat span{font-weight:650}.online-player-stat small{color:#667085;font-size:11px}.online-player-stat[data-stale]{border-color:#fecdca;background:#fff6ed}.online-player-stat[data-stale] small::after{content:' · 刷新失败'}"
        ...
        ".dot{color:#12b76a}.muted{color:#98a2b3}.notice{padding:10px 12px;border-radius:7px;margin-bottom:14px}.ok{background:#ecfdf3;color:#027a48}.error{background:#fef3f2;color:#b42318}.account-wallet{display:flex;align-items:center;gap:8px 12px;flex-wrap:wrap}.account-wallet-actions{display:flex;gap:7px;flex-wrap:wrap;margin-left:auto}.account-wallet .inline{margin:0}.account-wallet .inline input{min-width:160px}.wcoin-debit{background:#b54708}.wcoin-debit:hover{background:#93370d}.account-security{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin:0 0 16px;padding:14px;border:1px solid #fecdca;border-radius:9px;background:#fffbfa}.account-security h3{margin:0 0 3px;font-size:15px;color:#912018}.account-security p{margin:0;color:#667085}.account-security form{flex:none}.danger{background:#b42318}.danger:hover{background:#912018}@media(max-width:620px){.account-wallet-actions{width:100%%;margin-left:0}.account-wallet .inline{flex:1 1 100%%}.account-wallet .inline input{min-width:0}.account-security{display:grid}.account-security form,.account-security button{width:100%%}}"
        ...
        "<a class=\"tab\" href=\"/?tab=risk\">风险管理</a></nav><div class=\"grid\">"
        "<aside class=\"card\"><h2>账号（%u）</h2><div class=\"online-player-stat\" data-online-player-stat aria-live=\"polite\"><strong data-online-player-count>%u</strong><span>在线玩家</span><small>实时会话统计 · 每 10 秒刷新</small></div><form class=\"account-search\" data-account-search-form role=\"search\"><input data-account-search maxlength=\"63\" placeholder=\"搜索账号名\" aria-label=\"搜索账号名\" value=\"",
        accountTotal, onlinePlayerTotal);
    ...
    if (accountState != NULL)
    {
        ...
        if (vm_mock_service_account_wallet_read(selectedAccount, false, &accountWcoin))
        {
            vm_mock_admin_text_appendf(&page,
                ...
                "<div class=\"account-wallet-actions\"><form class=\"inline\" method=\"post\" action=\"/action\">"
            ...
            vm_mock_admin_text_appendf(&page,
                ...
                "<button type=\"submit\">加 W 币</button></form>"
                "<form class=\"inline\" method=\"post\" action=\"/action\" data-admin-confirm=\"确认减少该账号的 W 币？余额不足时不会扣除。\">"
                "<input type=\"hidden\" name=\"action\" value=\"remove-wcoin\">"
                "<input type=\"hidden\" name=\"account\" value=\"");
            vm_mock_admin_text_append_html(&page, selectedAccount);
            vm_mock_admin_text_appendf(&page,
                "\"><input type=\"number\" name=\"amount\" min=\"1\" max=\"4294967295\" placeholder=\"减少 W 币\" required>"
                "<button class=\"wcoin-debit\" type=\"submit\">减 W 币</button></form></div></div>");
        }
        else
        {
            vm_mock_admin_text_appendf(&page,
    ...
    if (accountState != NULL)
    {
        for (u32 i = 0; i < g_vm_net_mock_role_db.roleCount; ++i)
        {
            ...
            vm_mock_admin_role_timed_effect_list timedEffects;
            bool timedEffectsLoaded = vm_mock_admin_role_timed_effects_load(
                selectedAccount, role->roleId, &timedEffects);
            ...
            vm_mock_admin_render_role_operation_modal(
                &page, selectedAccount, role, roleNameUtf8, &timedEffects,
                timedEffectsLoaded, resetSceneFiles, resetSceneCount);
            vm_mock_admin_text_appendf(&page, "</td></tr>");
        }
...
/* The browser refreshes this small authenticated payload instead of
 * re-rendering the account directory or querying durable account rows. */
static int vm_mock_admin_handle_online_player_stats_request(
    vm_mock_service_socket client)
{
    char response[96];
    int written = snprintf(response, sizeof(response),
                           "{\"online_players\":%u}\n",
                           vm_mock_admin_online_player_count());
    ...
    return vm_mock_admin_send_response(
        client, "200 OK", "application/json; charset=utf-8",
...
static bool vm_mock_admin_sce_string_field_equals(
    const u8 *payload, u32 payloadLen, u32 lengthOffset, u16 field,
    const char *text)
{
    size_t textLen = 0;

    if (payload == NULL || text == NULL || lengthOffset < 4u ||
        lengthOffset >= payloadLen ||
        vm_net_mock_read_le16_at(payload, lengthOffset - 4u) != 3u ||
        vm_net_mock_read_le16_at(payload, lengthOffset - 2u) != field)
    {
        return false;
    }
    textLen = strlen(text);
    return textLen != 0 && textLen <= 0xffu &&
           payload[lengthOffset] == textLen &&
           lengthOffset + 1u + (u32)textLen <= payloadLen &&
           memcmp(payload + lengthOffset + 1u, text, textLen) == 0;
}

/* Create a changed decoded payload by replacing one exactly captured SCE
 * string field.  The caller owns the returned buffer.  A matching old/new
 * value is a valid no-op, distinguished through changedOut. */
static bool vm_mock_admin_sce_replace_string_field_payload(
    const u8 *payload, u32 payloadLen, u32 lengthOffset, u16 field,
    const char *oldText, const char *newText, u8 **newPayloadOut,
    u32 *newPayloadLenOut, bool *changedOut)
{
    size_t oldLen = 0;
    size_t newLen = 0;
    u8 *newPayload = NULL;
    ...
    if (changedOut)
        *changedOut = false;
    if (payload == NULL || oldText == NULL || newText == NULL ||
        newPayloadOut == NULL || newPayloadLenOut == NULL ||
        !vm_mock_admin_sce_string_field_equals(payload, payloadLen,
                                                lengthOffset, field, oldText))
    {
        return false;
    }
    oldLen = strlen(oldText);
    newLen = strlen(newText);
    if (newLen == 0 || newLen > 0xffu)
        return false;
    if (strcmp(oldText, newText) == 0)
        return true;
    oldFieldLen = payload[lengthOffset];
    if (newLen > oldLen &&
        payloadLen > UINT32_MAX - (u32)(newLen - oldLen))
    {
        return false;
    }
    newPayloadLen = payloadLen - oldFieldLen + (u32)newLen;
    newPayload = (u8 *)malloc(newPayloadLen);
    if (newPayload == NULL)
        return false;
    memcpy(newPayload, payload, lengthOffset);
    newPayload[lengthOffset] = (u8)newLen;
    memcpy(newPayload + lengthOffset + 1u, newText, newLen);
    memcpy(newPayload + lengthOffset + 1u + newLen,
           payload + lengthOffset + 1u + oldFieldLen,
           payloadLen - (lengthOffset + 1u + oldFieldLen));
    *newPayloadOut = newPayload;
    *newPayloadLenOut = newPayloadLen;
    if (changedOut)
        *changedOut = true;
    return true;
}

static bool vm_mock_admin_sce_encode_payload(u8 type, const u8 *payload,
                                             u32 payloadLen, u8 **rawOut,
                                             u32 *rawLenOut)
{
    u8 *encoded = NULL;
    u32 encodedLen = 0;
    ...
    if (rawLenOut)
        *rawLenOut = 0;
    if (payload == NULL || payloadLen == 0 || rawOut == NULL ||
        rawLenOut == NULL)
    {
        return false;
    }
    ...
    else if (type == 2u)
    {
        ...
        if (payloadLen > UINT32_MAX - 9u ||
            payloadLen > UINT32_MAX - 126u ||
            payloadLen + 9u > UINT32_MAX - (payloadLen + 126u) / 127u)
        {
            goto done;
        }
        encodedCap = 9u + payloadLen + (payloadLen + 126u) / 127u;
        encoded = (u8 *)malloc(encodedCap);
        if (encoded == NULL ||
            !vm_net_mock_scene_battle_monster_lzss_literal_encode(
                payload, payloadLen, encoded, encodedCap, &encodedLen) ||
...
static bool vm_mock_admin_sce_payload_has_header(const u8 *payload,
                                                 u32 payloadLen)
{
    u32 searchEnd = payloadLen < 32u ? payloadLen : 32u;

    if (payload == NULL || searchEnd < 4u)
        return false;
    for (u32 offset = 0; offset + 4u <= searchEnd; ++offset)
    {
        if (memcmp(payload + offset, "SCE2", 4) == 0)
            return true;
    }
    return false;
}

static bool vm_mock_admin_sce_copy_target_is_valid(const char *sourceScene,
                                                    const char *targetScene)
{
    return sourceScene != NULL && targetScene != NULL &&
           vm_net_mock_scene_name_is_safe(sourceScene) &&
           /* A new copy must not exist yet, so use the resource-leaf safety
            * predicate here rather than scene_name_is_safe(), whose contract
            * intentionally includes an existence check. */
           vm_net_mock_update_name_is_safe(targetScene) &&
           vm_net_mock_str_ends_with(sourceScene, ".sce") &&
           vm_net_mock_str_ends_with(targetScene, ".sce") &&
           strlen(sourceScene) < 64u && strlen(targetScene) < 64u &&
           strcmp(sourceScene, targetScene) != 0;
}

/* A copy retains the complete source container verbatim.  It validates both
 * decoded source and target before publishing the new resource key, so no SCE
 * record is guessed or rebuilt just to give the copy a different filename. */
static bool vm_mock_admin_sce_copy_resource(const char *sourceScene,
                                            const char *targetScene,
                                            const char **errorOut,
                                            bool *contentChangedOut)
{
    char sourcePath[1200];
    char targetPath[1200];
    u8 *sourceRaw = NULL;
    u32 sourceRawLen = 0;
    u8 *sourcePayload = NULL;
    ...
    if (!vm_mock_admin_sce_copy_target_is_valid(sourceScene, targetScene) ||
        vm_net_mock_scene_resource_exists(targetScene) ||
        !vm_net_mock_update_resource_path(sourceScene, sourcePath,
                                          sizeof(sourcePath)) ||
        !vm_net_mock_update_resource_path(targetScene, targetPath,
                                          sizeof(targetPath)) ||
        vm_host_file_exists(targetPath) ||
        !vm_mock_admin_read_raw_resource_file(sourcePath, &sourceRaw,
                                              &sourceRawLen) ||
        !vm_mock_admin_load_data_payload(sourceScene, ".sce", &sourcePayload,
                                         &sourcePayloadLen, &sourceType) ||
        (sourceType != 1u && sourceType != 2u) ||
        !vm_mock_admin_sce_payload_has_header(sourcePayload, sourcePayloadLen))
    {
        if (errorOut)
            *errorOut = "SCE 文件名无效、目标已存在或源资源不可读取";
        goto done;
    }
    if (!vm_mock_admin_write_resource_atomic(targetPath, sourceRaw,
                                             sourceRawLen))
    {
        if (errorOut)
            *errorOut = "新 SCE 原子写入失败，原场景未改动";
        goto done;
    }
    if (!vm_mock_admin_read_raw_resource_file(targetPath, &verifyRaw,
                                              &verifyRawLen) ||
        verifyRawLen != sourceRawLen ||
        memcmp(verifyRaw, sourceRaw, sourceRawLen) != 0 ||
        !vm_mock_admin_load_data_payload(targetScene, ".sce", &verifyPayload,
                                         &verifyPayloadLen, &verifyType) ||
        verifyType != sourceType || verifyPayloadLen != sourcePayloadLen ||
        memcmp(verifyPayload, sourcePayload, sourcePayloadLen) != 0)
    {
        removed = remove(targetPath) == 0;
        if (errorOut)
            *errorOut = removed ? "新 SCE 校验失败，已删除未发布的副本"
                                : "新 SCE 校验失败，删除未发布副本也失败";
        goto done;
    }
    publishNames[0] = targetScene;
    if (!vm_net_mock_content_update_publish_files(publishNames, 1,
                                                  &publishError, &changed))
    {
        removed = remove(targetPath) == 0;
        if (errorOut)
            *errorOut = removed ? "内容更新发布失败，已删除未发布的副本"
                                : "内容更新发布失败，删除未发布副本也失败";
        goto done;
    }
    if (contentChangedOut)
        *contentChangedOut = changed;
    printf("[info][mock-admin] sce_copy source=%s target=%s raw=%u publish=WT18/9+18/8->18/7\n",
           sourceScene, targetScene, sourceRawLen);
    ok = true;
...
static bool vm_mock_admin_sce_location_title_replace(
    const char *scene, const vm_mock_admin_scene_portal *portal,
    const char *newTitle, const char **errorOut, bool *contentChangedOut)
{
    char path[1200];
    u8 *previousRaw = NULL;
    ...
    if (contentChangedOut)
        *contentChangedOut = false;
    memset(path, 0, sizeof(path));
    if (scene == NULL || portal == NULL || newTitle == NULL ||
        portal->kind != VM_MOCK_ADMIN_PORTAL_NAMED ||
        !vm_net_mock_scene_name_is_safe(scene) ||
        !vm_net_mock_str_ends_with(scene, ".sce") ||
        newTitle[0] == 0 || strlen(newTitle) >= sizeof(portal->displayName) ||
        portal->displayNameLengthOffset == 0 ||
    ...
    {
        if (errorOut)
            *errorOut = "场景名称参数无效或权威 SCE 资源不可读取";
        goto done;
    }
    if (!vm_mock_admin_sce_replace_string_field_payload(
            payload, payloadLen, portal->displayNameLengthOffset, 0x16u,
            portal->displayName, newTitle, &newPayload, &newPayloadLen,
            &fieldChanged))
    {
        if (errorOut)
            *errorOut = "内置场景名称字段已变化；请刷新场景后重试";
        goto done;
    }
    if (!fieldChanged)
    {
    ...
    if (!vm_mock_admin_sce_encode_payload(type, newPayload, newPayloadLen,
                                          &raw, &rawLen))
    {
        if (errorOut)
            *errorOut = "SCE 压缩封装失败";
    ...
    if (!vm_mock_admin_write_resource_atomic(path, raw, rawLen) ||
        ...
        !vm_mock_admin_sce_string_field_equals(
            verifyPayload, verifyPayloadLen, portal->displayNameLengthOffset,
            0x16u, newTitle))
    {
    ...
    if (contentChangedOut)
        *contentChangedOut = changed;
    printf("[info][mock-admin] sce_location_title_save scene=%s offset=%u raw=%u publish=WT18/9+18/8->18/7\n",
           scene, portal->displayNameLengthOffset, rawLen);
    ok = true;
...
static void vm_mock_admin_handle_sce_resource_action(
    vm_mock_service_socket client, const char *action, const char *body)
{
    ...
    {
        vm_mock_admin_redirect_content_section(
            client, sceneUtf8, "npcs", "error", "场景参数无效或 SCE 不存在");
        return;
    }
    if (strcmp(action, "copy-sce-resource") == 0)
    {
        if (!vm_mock_admin_form_value(body, "new_scene_file", valueUtf8,
                                      sizeof(valueUtf8)) ||
            !vm_mock_admin_utf8_to_gbk_text(valueUtf8, valueGbk,
                                             sizeof(valueGbk), false) ||
            !vm_mock_admin_sce_copy_target_is_valid(runtimeScene, valueGbk))
        {
            vm_mock_admin_redirect_content_section(
                client, sceneUtf8, "npcs", "error",
                "新 SCE 文件名无效；请填写未占用的 .sce 文件名");
            return;
        }
        if (!vm_mock_admin_sce_copy_resource(runtimeScene, valueGbk, &error,
                                             &changed))
        {
            vm_mock_admin_redirect_content_section(
                client, sceneUtf8, "npcs", "error",
                error ? error : "SCE 复制失败");
            return;
        }
        vm_mock_admin_redirect_content_section(
            client, valueUtf8, "npcs", "ok",
            changed ? "新的 SCE 已完成原始字节校验并加入内容更新清单"
                    : "新的 SCE 已复制；内容更新版本未变化");
        return;
    }
    if (strcmp(action, "save-sce-location-title") != 0 ||
        !vm_mock_admin_form_value(body, "scene_title", valueUtf8,
                                  sizeof(valueUtf8)) ||
        !vm_mock_admin_utf8_to_gbk_text(valueUtf8, valueGbk,
                                         sizeof(valueGbk), false) ||
        valueGbk[0] == 0 || strlen(valueGbk) >= sizeof(valueGbk))
    {
        vm_mock_admin_redirect_content_section(
            client, sceneUtf8, "npcs", "error",
            "内置场景名称无效；请填写不超过 63 个 GBK 字节的名称");
        return;
    }
    portalCount = vm_mock_admin_collect_scene_portals(
        runtimeScene, portals, VM_MOCK_ADMIN_PREVIEW_PORTAL_MAX, NULL);
    for (u32 i = 0; i < portalCount; ++i)
    {
        if (portals[i].kind == VM_MOCK_ADMIN_PORTAL_NAMED &&
            portals[i].displayName[0] != 0 &&
            portals[i].displayNameLengthOffset != 0)
        {
            titlePortal = &portals[i];
            break;
        }
    }
    if (titlePortal == NULL)
    {
        vm_mock_admin_redirect_content_section(
            client, sceneUtf8, "npcs", "error",
            "该 SCE 没有可精确编辑的内置场景名称字段");
        return;
    }
    if (!vm_mock_admin_sce_location_title_replace(runtimeScene, titlePortal,
                                                  valueGbk, &error, &changed))
    {
        vm_mock_admin_redirect_content_section(
            client, sceneUtf8, "npcs", "error",
            error ? error : "内置场景名称保存失败");
        return;
    }
    vm_mock_admin_redirect_content_section(
        client, sceneUtf8, "npcs", "ok",
        changed ? "内置场景名称已校验并加入内容更新清单"
                : "内置场景名称未变化，未重复发布内容更新");
}
...
{
    char statusEncoded[64];
    /* UTF-8 Chinese bytes expand to three URL-encoded bytes each.  Monster
     * bulk actions return detailed summaries, so keep this redirect chain
     * large enough for the full message instead of silently truncating it. */
    char messageEncoded[2048];
    char location[2304];
...
static void vm_mock_admin_redirect_risk_directory(
    vm_mock_service_socket client, const char *directory, u32 page,
    const char *status, const char *message)
{
    char statusEncoded[64];
    char messageEncoded[768];
    char location[1100];
    const char *riskKind = directory != NULL && strcmp(directory, "ips") == 0
                               ? "ips"
                               : "roles";
    ...
    snprintf(location, sizeof(location),
             VM_MOCK_ADMIN_ROOT_PATH
             "?tab=risk&risk_kind=%s&page=%u&status=%s&message=%s",
             riskKind, page ? page : 1, statusEncoded, messageEncoded);
    vm_mock_admin_send_location(client, location, NULL);
}
...
static void vm_mock_admin_redirect_risk(vm_mock_service_socket client,
                                        ...
                                        const char *message)
{
    vm_mock_admin_redirect_risk_directory(client, "roles", page, status,
                                          message);
}

static void vm_mock_admin_redirect_risk_ip(vm_mock_service_socket client,
                                           u32 page,
                                           const char *status,
                                           const char *message)
{
    vm_mock_admin_redirect_risk_directory(client, "ips", page, status,
                                          message);
}
                                          ...
                                          const char *message)
{
    ...
    snprintf(location, sizeof(location),
             VM_MOCK_ADMIN_ROOT_PATH
             "?tab=content&content_kind=actor&actor=%s&status=%s&message=%s",
...
static bool vm_mock_admin_form_drop_rate_basis_points(const char *body,
                                                       const char *field,
                                                       u16 *valueOut)
{
    char textValue[32];
    ...
    memset(textValue, 0, sizeof(textValue));
    return vm_mock_admin_form_value(body, field, textValue, sizeof(textValue)) &&
           vm_net_mock_parse_drop_rate_basis_points(
               textValue, strlen(textValue), valueOut);
}
                                              ...
                                              const char *body)
{
    ...
    if (strcmp(action, "save-task") != 0 ||
        ...
        !vm_mock_admin_form_value(body, "requirement_scene1", requirementScene1Utf8,
                                  sizeof(requirementScene1Utf8)) ||
        !vm_mock_admin_form_value(body, "requirement_scene2", requirementScene2Utf8,
                                  sizeof(requirementScene2Utf8)) ||
    ...
    createNew = strcmp(createNewText, "1") == 0;
    if (createNew)
    {
        vm_net_mock_task_definition tasks[VM_NET_MOCK_TASK_CATALOG_MAX];
        u32 taskCount = vm_net_mock_task_admin_list(
            tasks, VM_NET_MOCK_TASK_CATALOG_MAX);

        if (!vm_mock_admin_find_first_unused_task_id(tasks, taskCount,
                                                      &taskId))
        {
            vm_mock_admin_redirect_tasks(client, 0, "error",
                                         "没有可分配的任务 ID");
            return;
        }
        task.taskId = taskId;
...
#undef VM_TASK_FORM_U32
    ...
    if (task.requirementType1 > 2 || task.requirementType2 > 2 ||
        ...
        !vm_mock_admin_utf8_to_gbk_task_text(requirementScene1Utf8,
                                             task.requirementScene1,
                                             sizeof(task.requirementScene1), true) ||
        !vm_mock_admin_utf8_to_gbk_task_text(requirementScene2Utf8,
                                             task.requirementScene2,
                                             sizeof(task.requirementScene2), true) ||
                                                 ...
                                                 const char *body)
{
    ...
    if (strcmp(action, "assign-monster-equipment-drops") == 0)
    {
        vm_net_mock_monster_equipment_drop_assignment assignment;
        u16 qualityRates[3] = {0, 0, 0};
        char message[384];

        memset(&assignment, 0, sizeof(assignment));
        memset(message, 0, sizeof(message));
        if (!vm_mock_admin_form_drop_rate_basis_points(
                body, "quality0_drop_rate", &qualityRates[0]) ||
            !vm_mock_admin_form_drop_rate_basis_points(
                body, "quality1_drop_rate", &qualityRates[1]) ||
            !vm_mock_admin_form_drop_rate_basis_points(
                body, "quality2_drop_rate", &qualityRates[2]) ||
            qualityRates[0] == 0 || qualityRates[1] == 0 ||
            qualityRates[2] == 0)
        {
            vm_mock_admin_redirect_monster(
                client, row.enemyId, "error",
                "品质 0、1、2 的装备掉落概率都必须为 0.01 到 100，且最多两位小数");
            return;
        }
        if (!vm_net_mock_monster_admin_assign_equipment_drops(
                qualityRates, &assignment, &error))
        {
            vm_mock_admin_redirect_monster(
                client, row.enemyId, "error",
                error ? error : "智能装备掉落分配失败");
            return;
        }
        snprintf(message, sizeof(message),
                 "分配完成：品质0=%u、品质1=%u、品质2=%u；跳过0=%u、跳过1=%u、跳过2=%u；"
                 "排除场景怪=%u，任务材料=%u，名称匹配=%u",
                 assignment.equipmentByQuality[0],
                 assignment.equipmentByQuality[1],
                 assignment.equipmentByQuality[2],
                 assignment.equipmentSkippedByQuality[0],
                 assignment.equipmentSkippedByQuality[1],
                 assignment.equipmentSkippedByQuality[2],
                 assignment.sceneBattleMonsterCount,
                 assignment.taskDropsPreserved,
                 assignment.strongNameMatches);
        vm_mock_admin_redirect_monster(client, row.enemyId, "ok", message);
        return;
    }
    if (strcmp(action, "batch-configure-monster-drops") == 0)
    {
        u32 itemIds[VM_NET_MOCK_MONSTER_DROP_MAX];
        u32 itemCount = 0;
        u32 levelMin = 0;
        u32 levelMax = 0;
        u16 addRate = 0;
        ...
        char familyText[16];
        char message[384];
        vm_net_mock_monster_drop_batch_mode mode;
        vm_net_mock_monster_drop_batch_family dropFamily;
        vm_net_mock_monster_drop_batch_result result;

        memset(itemIds, 0, sizeof(itemIds));
        memset(modeText, 0, sizeof(modeText));
        memset(familyText, 0, sizeof(familyText));
        memset(message, 0, sizeof(message));
        memset(&result, 0, sizeof(result));
        if (!vm_mock_admin_form_value(body, "drop_batch_mode", modeText,
                                      sizeof(modeText)) ||
            !vm_mock_admin_form_value(body, "drop_batch_family", familyText,
                                      sizeof(familyText)) ||
            !vm_mock_admin_form_u32(body, "drop_batch_level_min", 0xffu,
                                    &levelMin) ||
            !vm_mock_admin_form_u32(body, "drop_batch_level_max", 0xffu,
                                    &levelMax) ||
            levelMin == 0 || levelMax == 0 || levelMin > levelMax ||
            !vm_mock_admin_form_u32_csv(
                body, "drop_batch_item_ids", itemIds,
                VM_NET_MOCK_MONSTER_DROP_MAX, &itemCount))
        {
            vm_mock_admin_redirect_monster(
                client, row.enemyId, "error",
                "批量掉落需要选择 1 至 12 件有效物品，并填写 1 到 255 的等级范围");
            return;
        }
        if (strcmp(modeText, "add") == 0)
            mode = VM_NET_MOCK_MONSTER_DROP_BATCH_ADD;
        else if (strcmp(modeText, "remove") == 0)
        ...
        if (strcmp(familyText, "all") == 0)
            dropFamily = VM_NET_MOCK_MONSTER_DROP_BATCH_FAMILY_ALL;
        else if (strcmp(familyText, "boss") == 0)
            dropFamily = VM_NET_MOCK_MONSTER_DROP_BATCH_FAMILY_BOSS;
        else if (strcmp(familyText, "nonboss") == 0)
            dropFamily = VM_NET_MOCK_MONSTER_DROP_BATCH_FAMILY_NON_BOSS;
        else
        {
            vm_mock_admin_redirect_monster(client, row.enemyId, "error",
                                           "首领类型筛选无效");
        ...
        if (mode == VM_NET_MOCK_MONSTER_DROP_BATCH_ADD &&
            !vm_mock_admin_form_drop_rate_basis_points(
                body, "drop_batch_rate", &addRate))
        {
            vm_mock_admin_redirect_monster(
                client, row.enemyId, "error",
                "新增掉落概率必须为 0.01 到 100，且最多两位小数");
            return;
        }
        if (!vm_net_mock_monster_admin_apply_drop_batch(
                itemIds, itemCount, mode, dropFamily, levelMin, levelMax,
                addRate, &result, &error))
        {
            vm_mock_admin_redirect_monster(
                client, row.enemyId, "error",
                error ? error : "批量掉落配置失败");
            return;
        }
        if (mode == VM_NET_MOCK_MONSTER_DROP_BATCH_ADD)
        {
            snprintf(message, sizeof(message),
                     "批量增加完成：命中 %u 只，更新 %u 只，新增 %u 条；已存在 %u 条，"
                     "槽位不足跳过 %u 条，保护场景战斗怪 %u 只",
                     result.matchedMonsters, result.changedMonsters,
                     result.addedDrops, result.alreadyPresent,
                     result.capacitySkipped, result.sceneBattleExcluded);
        }
        else
        {
            snprintf(message, sizeof(message),
                     "批量删除完成：命中 %u 只，更新 %u 只，删除 %u 条；"
                     "保护场景战斗怪 %u 只",
                     result.matchedMonsters, result.changedMonsters,
                     result.removedDrops, result.sceneBattleExcluded);
        }
        vm_mock_admin_redirect_monster(client, row.enemyId, "ok", message);
        return;
    }
    ...
    for (u8 slot = 0; slot < VM_NET_MOCK_MONSTER_DROP_MAX; ++slot)
    {
        ...
        if (!vm_mock_admin_form_u32(body, itemField, 0xffffffffu, &itemId) ||
            !vm_mock_admin_form_drop_rate_basis_points(
                body, rateField, &rateBasisPoints))
        {
            vm_mock_admin_redirect_monster(client, row.enemyId, "error",
                                           "掉落表单字段不完整或数值越界");
            return;
        }
        if (itemId == 0 && rateBasisPoints == 0)
            continue;
        if (itemId == 0 || rateBasisPoints == 0 ||
        ...
        row.drops[row.dropCount].rateBasisPoints = rateBasisPoints;
        ++row.dropCount;
    }
                                            ...
                                            const char *body)
{
    ...
    u32 y = 0;
    u32 kind = 0;
    u32 selectedTaskIds[VM_NET_MOCK_DYNAMIC_NPC_TASK_MAX];
    ...
    memset(taskBindings, 0, sizeof(taskBindings));
    memset(selectedTaskIds, 0, sizeof(selectedTaskIds));
    if (!vm_mock_admin_scene_from_form(body, sceneUtf8, sizeof(sceneUtf8),
                                       runtimeScene, sizeof(runtimeScene)))
    {
    ...
    if (strcmp(action, "toggle-npc") == 0)
    {
        ...
        for (u32 i = 0; i < rowCount; ++i)
        {
            ...
            {
                seed = rows[i].seed;
                memcpy(taskBindings, rows[i].taskBindings,
                       sizeof(taskBindings));
                taskBindingCount = rows[i].taskBindingCount;
                found = true;
                break;
            }
        }
        ...
        if (!found || !vm_net_mock_dynamic_npc_admin_save(
                          runtimeScene, &seed, taskBindings, taskBindingCount,
                          enabled, NULL, 0, false, false,
                          &error))
        {
            vm_mock_admin_redirect_content(
                client, sceneUtf8, "error",
    ...
    if (!vm_mock_admin_form_value(body, "task_ids", taskIds,
                                  sizeof(taskIds)) ||
        (taskIds[0] != 0 &&
         !vm_mock_admin_form_u32_csv(
             body, "task_ids", selectedTaskIds,
             VM_NET_MOCK_DYNAMIC_NPC_TASK_MAX, &taskBindingCount)))
    {
        vm_mock_admin_redirect_content(client, sceneUtf8, "error",
                                       "任务选择格式无效、重复或超过上限");
        return;
    }
    for (u32 taskIndex = 0; taskIndex < taskBindingCount; ++taskIndex)
    {
        if (vm_net_mock_task_catalog_find_by_id(selectedTaskIds[taskIndex]) == NULL)
        {
            vm_mock_admin_redirect_content(client, sceneUtf8, "error",
                                           "绑定任务不存在或已停用");
            return;
        }
        taskBindings[taskIndex].taskId = selectedTaskIds[taskIndex];
        taskBindings[taskIndex].repeatPolicy = (u8)taskRepeatPolicy;
    }
    if (taskBindingCount == 0 &&
        taskRepeatPolicy != VM_NET_MOCK_TASK_REPEAT_NEVER)
    {
        vm_mock_admin_redirect_content(client, sceneUtf8, "error",
    ...
    kind = serviceOptionCount != 0 ? serviceOptions[0].kind
                                   : VM_NET_MOCK_NPC_KIND_NORMAL;
    hasInstanceTeleport = vm_net_mock_npc_service_options_has_kind(
    ...
    if (hasInstanceTeleport || hasInstanceChallenge)
    {
        ...
        if (!vm_mock_admin_form_u32(body, "instance_min_level", 0xffu,
                                    &instanceMinLevel) ||
            instanceMinLevel == 0)
        {
            vm_mock_admin_redirect_content(
        ...
        {
            vm_mock_admin_redirect_content(
                client, sceneUtf8, "error", "副本传送必须配置目标场景和落点");
            return;
        }
        if (hasInstanceTeleport &&
                 ...
                 runtimeScene, challengeEnemyId)))
        {
            vm_mock_admin_redirect_content(
                client, sceneUtf8, "error",
                "守关怪必须是当前场景中已启用的场景战斗怪");
            return;
        }
        if (hasInstanceTeleport &&
            (!vm_mock_admin_form_u32(body, "instance_spawn_enemy_id",
                                     0xffffu, &instanceSpawnEnemyId) ||
             (instanceSpawnEnemyId != 0 &&
              !vm_net_mock_scene_battle_monster_target_ready(
                  instanceRuntimeScene, instanceSpawnEnemyId))))
        {
            vm_mock_admin_redirect_content(
                client, sceneUtf8, "error",
                "目标场景刷新怪物必须是已启用且已部署的场景战斗怪；填 0 表示不刷新");
            return;
        }
    }
    ...
    seed.y = (u16)y;
    seed.kind = (u16)kind;
    seed.taskId = taskBindingCount != 0 ? taskBindings[0].taskId : 0;
    seed.taskRepeatPolicy = taskBindingCount != 0
    ...
    if (!vm_net_mock_dynamic_npc_admin_save(
            runtimeScene, &seed, taskBindings, taskBindingCount, true,
            serviceOptions, serviceOptionCount,
    ...
    vm_mock_service_socket client, const char *action, const char *body)
{
    ...
    if (strcmp(action, "save-scene-battle-monster") != 0 ||
        !vm_mock_admin_form_optional_u32(body, "monster_id", 0xffffu,
                                         &row.monsterId) ||
        !vm_mock_admin_form_optional_u32(body, "source_monster_id", 0xffffu,
                                         &sourceMonsterId) ||
        ...
        !vm_mock_admin_form_u32(
            body, "quantity",
            VM_NET_MOCK_SCENE_BATTLE_MONSTER_QUANTITY_MAX, &quantity) ||
        quantity == 0 ||
                                        ...
                                        sizeof(row.effectResource), false))
    {
        vm_mock_admin_redirect_scene_battle_monsters(
            client, sceneUtf8, "error",
            "怪物 ID、参考怪物、名称、本体 Actor、退场特效、坐标、数量、视觉提示或状态无效");
        return;
    }
    row.x = (u16)posX;
    row.y = (u16)posY;
    ...
    if (row.entryId == 0)
    {
        if (row.monsterId != 0)
        {
            if (sourceMonsterId != 0 ||
                !vm_net_mock_scene_battle_monster_reference_monster_get(
                    row.monsterId, &sourceMonster))
            {
                vm_mock_admin_redirect_scene_battle_monsters(
                    client, sceneUtf8, "error",
                    sourceMonsterId != 0
                        ? "复用已有怪物 ID 时不能同时选择参考怪物"
                        : "待复用的怪物 ID 不在当前怪物目录中");
                return;
            }
        }
        else
        {
                    ...
                    sourceMonsterId, &sourceMonster))
            {
                vm_mock_admin_redirect_scene_battle_monsters(
                    client, sceneUtf8, "error",
                    "请先选择要复用的怪物 ID，或选择可作为属性和掉落模板的参考怪物");
                return;
            }
    ...
    if (row.entryId == 0 && cloneReference)
    {
    ...
    {
        snprintf(cloneMessage, sizeof(cloneMessage),
                 "场景战斗怪草稿已新增：复用怪物 ID %u，可在其他坐标继续复用；请部署后客户端才会创建战斗节点",
                 row.monsterId);
    }
    vm_mock_admin_redirect_scene_battle_monsters(
        client, sceneUtf8, "ok",
        row.entryId == 0 ? cloneMessage :
                           "场景战斗怪草稿已保存；怪物 ID 已按当前选择更新，请部署后客户端才会更新战斗节点");
}
                                        ...
                                        const char *operatorAccountId)
{
    ...
    char effectKindText[32];
    char multiplierText[32];
    char expiresUnixText[32];
    char roleNameUtf8[128];
    char roleNameGbk[32];
    ...
    u32 enhanceLevel = 0;
    u32 roleId = 0;
    u32 timedEffectKind = 0;
    u32 timedEffectMultiplier = 0;
    u32 timedEffectExpiresUnix = 0;
    u16 itemSeq = 0;
    ...
    memset(effectKindText, 0, sizeof(effectKindText));
    memset(multiplierText, 0, sizeof(multiplierText));
    memset(expiresUnixText, 0, sizeof(expiresUnixText));
    memset(roleNameUtf8, 0, sizeof(roleNameUtf8));
    ...
    if (strcmp(action, "copy-sce-resource") == 0 ||
        strcmp(action, "save-sce-location-title") == 0)
    {
    ...
    if (strcmp(action, "assign-monster-equipment-drops") == 0 ||
        strcmp(action, "batch-configure-monster-drops") == 0 ||
        strcmp(action, "save-monster") == 0 ||
    ...
    if (strcmp(action, "clear-risk-ip") == 0)
    {
        char riskIp[VM_MOCK_SERVICE_LOGIN_IP_CAP];
        char pageText[16];
        char ipAuditTarget[64];
        char message[192];
        u32 pageNumber = 1;
        bool cleared = false;

        memset(riskIp, 0, sizeof(riskIp));
        memset(pageText, 0, sizeof(pageText));
        memset(ipAuditTarget, 0, sizeof(ipAuditTarget));
        memset(message, 0, sizeof(message));
        (void)vm_mock_admin_form_value(body, "page", pageText,
                                       sizeof(pageText));
        if (pageText[0] != 0 &&
        ...
        if (!vm_mock_admin_form_value(body, "risk_ip", riskIp,
                                      sizeof(riskIp)) ||
            !vm_mock_service_login_ip_is_valid(riskIp))
        {
            vm_mock_admin_redirect_risk_ip(client, pageNumber, "error",
                                           "来源 IP 参数无效");
            return;
        }
        if (!vm_mock_service_login_ip_clear_block(riskIp, &cleared))
        {
            vm_mock_admin_redirect_risk_ip(
                client, pageNumber, "error",
                "清除来源 IP 封锁失败，请检查数据库连接");
            return;
        }
        if (!cleared)
        {
        ...
        snprintf(ipAuditTarget, sizeof(ipAuditTarget), "ip:%s", riskIp);
        snprintf(operationDetail, sizeof(operationDetail),
                 "清除风险 IP 封锁和失败计数：%s", riskIp);
        if (!vm_mock_admin_operation_log_record(
                "clear-risk-ip", ipAuditTarget, 0, 0, 0, 0,
                operationDetail, operatorAccountId))
        {
            vm_mock_admin_redirect_risk_ip(
                client, pageNumber, "error",
                "风险 IP 已清除，但操作日志写入失败，请检查数据库");
            return;
        }
        snprintf(message, sizeof(message),
                 "已清除来源 IP %s 的封锁及失败计数；后续凭据失败将从第 1 次开始统计",
                 riskIp);
        vm_mock_admin_redirect_risk_ip(client, pageNumber, "ok", message);
        return;
    }
    if (strcmp(action, "ban-risk-account") == 0)
    {
        ...
        if (ok)
        {
            ...
            snprintf(operationDetail, sizeof(operationDetail),
                     "封禁风险账号，断开 %u 个游戏会话、注销 %u 个用户中心会话",
    ...
    if (strcmp(action, "set-role-name") == 0)
    {
        ...
        ok = vm_mock_service_account_set_role_name(account, role, roleNameGbk,
                                                   &error);
        if (ok && (error == NULL || strcmp(error, "角色名称没有变化") != 0))
        {
        ...
        strcmp(action, "remove-wcoin") == 0)
    {
        ...
        else if (strcmp(action, "add-wcoin") == 0)
            ok = vm_mock_service_account_add_wcoin(account, amount, &error);
        else
            ok = vm_mock_service_account_remove_wcoin(account, amount, &error);
        if (ok)
        {
            roleId = 0;
            if (strcmp(action, "add-money") == 0)
                    ...
                    operatorAccountId))
            {
                vm_mock_admin_redirect(client, account, "error",
                                       strcmp(action, "remove-wcoin") == 0
                                           ? "W 币已减少，但操作日志写入失败，请检查数据库"
                                           : "余额已增加，但操作日志写入失败，请检查数据库");
                return;
            }
        }
        vm_mock_admin_redirect(client, account, ok ? "ok" : "error",
                               ok ? (strcmp(action, "add-money") == 0
                                         ? "普通钱币增加成功"
    ...
    if (strcmp(action, "grant-item") == 0)
    {
        ...
        if (ok)
        {
            ...
            (void)vm_net_mock_parse_u32_strict(role, &roleId);
            snprintf(operationDetail, sizeof(operationDetail),
                     "发放%s %s（ID %u）×%u，背包序号 %u",
                     catalogItem != NULL && catalogItem->isEquip ? "装备" : "物品",
    ...
    if (strcmp(action, "remove-role-timed-item-effect") == 0)
    {
        if (!vm_mock_admin_form_value(body, "role", role, sizeof(role)) ||
            !vm_mock_admin_form_value(body, "effect_kind", effectKindText,
                                      sizeof(effectKindText)) ||
            !vm_mock_admin_form_value(body, "item", itemText,
                                      sizeof(itemText)) ||
            !vm_mock_admin_form_value(body, "multiplier", multiplierText,
                                      sizeof(multiplierText)) ||
            !vm_mock_admin_form_value(body, "expires_unix", expiresUnixText,
                                      sizeof(expiresUnixText)) ||
            !vm_net_mock_parse_u32_strict(role, &roleId) || roleId == 0 ||
            !vm_net_mock_parse_u32_strict(effectKindText, &timedEffectKind) ||
            timedEffectKind == 0 ||
            timedEffectKind > VM_NET_MOCK_ROLE_ITEM_EFFECT_EVENT_ATTACK_DEFENSE ||
            !vm_net_mock_parse_u32_strict(itemText, &itemId) || itemId == 0 ||
            !vm_net_mock_parse_u32_strict(multiplierText,
                                          &timedEffectMultiplier) ||
            timedEffectMultiplier > 255 ||
            !vm_net_mock_parse_u32_strict(expiresUnixText,
                                          &timedEffectExpiresUnix) ||
            timedEffectExpiresUnix == 0)
        {
            vm_mock_admin_redirect(client, account, "error",
                                   "时效道具效果参数无效");
            return;
        }
        ok = vm_mock_admin_role_timed_effect_remove(
            account, roleId, (u8)timedEffectKind, itemId,
            (u8)timedEffectMultiplier, timedEffectExpiresUnix, &error);
        if (ok)
        {
            const vm_net_mock_shop_catalog_item *catalogItem =
                vm_net_mock_find_shop_catalog_item(itemId);
            ...
            vm_mock_admin_role_timed_effect_bonus_text(
                (u8)timedEffectKind, (u8)timedEffectMultiplier, bonusText,
                sizeof(bonusText));
            snprintf(operationDetail, sizeof(operationDetail),
                     "移除生效中的%s：%s（ID %u，%s，原到期时间 %u）",
                     vm_mock_admin_role_timed_effect_label(
                         (u8)timedEffectKind),
                     itemNameUtf8[0] ? itemNameUtf8 : "未命名物品", itemId,
                     bonusText, timedEffectExpiresUnix);
            if (!vm_mock_admin_operation_log_record(
                    "remove-role-timed-item-effect", account, roleId, itemId,
                    1, timedEffectMultiplier, operationDetail,
                    operatorAccountId))
            {
                vm_mock_admin_redirect(
                    client, account, "error",
                    "时效道具效果已移除，但操作日志写入失败，请检查数据库");
                return;
            }
        }
        vm_mock_admin_redirect(client, account, ok ? "ok" : "error",
                               ok ? "时效道具效果已移除" :
                                    (error ? error : "时效道具效果删除失败"));
        return;
    }
...
/* This is a process-local live-role count, not an account-table count.  A
 * player is included only after a role has completed the normal online
 * transition; disconnected, title-only, and stale unbound sessions do not
 * inflate the administration statistic. */
static u32 vm_mock_admin_online_player_count(void)
{
    ...
    while (session != NULL)
    {
        if (session->roleOnline && session->onlineRoleId != 0 &&
                                          ...
                                          u32 contentLength)
{
    ...
    if (strcmp(target, VM_MOCK_ADMIN_ONLINE_PLAYER_STATS_PATH) == 0)
    {
        if (strcmp(method, "GET") != 0)
        {
            vm_mock_admin_send_response(client, "405 Method Not Allowed", NULL,
        ...
        return vm_mock_admin_handle_online_player_stats_request(client);
    }
