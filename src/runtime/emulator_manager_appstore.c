...
bool vm_appstore_handle(u32 address)
{
    ...
    if (!vm_net_mock_env_u32("CBE_STUB_APPSTORE_MANAGER", 1))
    {
        printf("[call]vmAppStoreManager idx=%u\n", idx);
        assert(0);
    }

    vm_net_trace("vmAppStoreManager_stub idx=%u addr=%08x r0=%08x r1=%08x r2=%08x r3=%08x lr=%08x last=%08x tick=%u\n",
