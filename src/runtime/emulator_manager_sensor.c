...
static u8 g_vmSensorManagerLogged[256];
...
bool vm_manager_sensor_handle(u32 address)
{
    ...
    if (!vm_net_mock_env_u32("CBE_STUB_SENSOR_MANAGER", 1))
    {
        printf("[impl]vmSensorManager调用位置:%d\n", idx);
        assert(0);
    }

    if (idx < sizeof(g_vmSensorManagerLogged))
    {
        if (!g_vmSensorManagerLogged[idx])
        {
            g_vmSensorManagerLogged[idx] = 1;
            vm_net_trace("vmSensorManager_stub idx=%u addr=%08x r0=%08x r1=%08x r2=%08x r3=%08x lr=%08x last=%08x tick=%u\n",
