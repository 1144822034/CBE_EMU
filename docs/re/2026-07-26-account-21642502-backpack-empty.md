...
修复把 `30/21` 每行的最大编码长度与客户端 64 格逻辑容量定义为
`VM_NET_MOCK_BACKPACK_CLIENT_ITEMINFO_MAX_BYTES`。该上限会随
`ParseEquipAttributes` 的公共属性流（当前最大 63 字节）推导，当前为 5059 字节；登录网格与 `17/1`
