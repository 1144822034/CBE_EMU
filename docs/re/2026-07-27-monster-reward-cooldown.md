...
冷却只影响怪物胜利奖励；场景挂机在上一场的原生收束回到场景后，才检查是否可开始下一
场。窗口未到期时保持在场景并延后续战，避免在客户端的自动战斗状态内制造无法表示的无
奖励终局。非终结动作、胜负、HP/MP 结算、装备耐久与自动恢复不受该等待影响。组队中
每个成员按自己的持久化角色身份独立领取。
...
切磋及其他未开启自动状态的流程有已验证的原生无奖励终局：最后一个 `4/6` 死亡动作
播放完毕后，服务端只下发 `4/11 { result=1,type=0 } + 4/9 { result=1 }`，不发送 `4/7`。
但这一对象族不能推广到自动挂机：`HandleServerBattleCmd(0x7BD0)` 先由
`4/11(type=0)` 清掉自动标志，随后 `4/9(result=1)` 因失去该标志不能进入自动收束
phase。因此若自动最后一击命中奖励冷却，不能构造“自动无奖励终局”。

场景挂机现在在上一场的 `4/9 → 25/5` 原生收束完成、下一场 `4/5` 尚未开始时读取 MySQL
窗口：

1. 窗口未到期时停留场景，仅延后下一次续战检查；不进入 `4/11(type=1)` 自动状态，
   也不构造 `4/6`、`4/7`、`4/11(type=0)` 或 `4/9`；
2. 窗口到期后才开始下一场，使该场正常走 `4/6 + 4/7` 获奖结算；
3. 非自动无奖励流程仍可使用既有的 `4/11 + 4/9`，不在此机制中变化。
...
- 同一角色首次击杀后 8 秒内再用挂机：上一场结束后停留场景，不发下一场 `4/5` 或任何
  战斗对象；窗口到期后才开始新场并走 `4/6 + 4/7`；
- 场景等待期间取消挂机：场景挂机续接应停止，不能延迟开始旧战斗；
