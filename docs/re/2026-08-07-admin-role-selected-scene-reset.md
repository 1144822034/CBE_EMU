...
账号管理中的角色行现在显示当前位置；“重置角色位置”复用后台的可搜索 SCE 场景选择
弹窗。选择器仍把精确的 UTF-8 文件显示名保存到表单的 `reset_scene` 隐藏选择框，提交
字段和服务端验证不变。选择器在角色操作弹窗之上显示；关闭或按 Esc 只关闭场景选择器，
不会误关外层角色操作弹窗。
...
`scripts/admin-role-item-picker-layering-regression.c` 还覆盖位置重置字段已绑定到共享
场景选择器，以及其与角色操作弹窗的嵌套关闭顺序。
