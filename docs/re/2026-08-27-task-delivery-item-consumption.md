...
每条 iteminfo 记录使用实际 `item_seq` 和扣除后的数量。普通物品行在使用前会按
`item.dsh` 的堆叠上限规范化；因此合法的客户端可见行可由 case-4 的 `u8` iteminfo
精确表示。保留构包前的范围检查，防止未来放宽资源堆叠上限后静默截断并造成客户端与
数据库分叉。
...
4. `item.dsh` 中任务物品的最大合法堆叠扣除一件后，精确下发对应余量。
...
工作区恢复可构建后，`mingw32-make -j2` 及
`task-delivery-item-consumption-regression` 均通过。回归的堆叠边界改为读取
`item.dsh` 的实际物品堆叠上限；直接构造 `255` 的同一行会被既有背包规范化逻辑拆成多个
客户端可见行，并不是 case-4 能收到的单行状态。
