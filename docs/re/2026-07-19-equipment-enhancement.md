...
- 玄晶强度（29/1 本地显示表，按当前强化等级 L 编码）：
  `required[*]=100`，`power[tier]=unit_rate(L,tier)`，使界面直接显示 100/33/11/3；
  成功后再附 `29/1` 刷新（`2026-08-01-enhance-client-display-unit-rate.md`）。
  勿再下发全局指数强度表（`2026-07-31-enhance-maxlevel-clamp-at-12.md`）。
- 成功率（权威：29/2 `value` / 29/3 掷骰）：
  - `+L→+(L+1)`：**目标级及以上**（`tier≥L+1`）→ 100%；每小一级 `floor(/3)`（33%、11%、3%…）
  - 例：`+5→+6` 六级晶 100%，五级晶 33%
  - `rate = min(100, Σ unit(tier)*count)`
  - 详见 `2026-08-01-enhance-same-tier-rate-div3.md`
- 铜币：紧凑曲线 `+0→100`，`+N→N×250`（与 29/1 显示表分离）
...
- [x] 选择玄晶后成功率和费用刷新（29/1 本地表按 unit_rate 编码；29/2 value/money）。
