...
- wt_kind: `26`、`7`
- wt_subtype: `1`、`7`、`11`
- fields: 首对象 `26/1 {hidebtn,dialog}`；出售成功后追加
  `7/7 {type=2,iteminfo}` 与 `7/11 {info}`。
...
- dispatch_case: action `1` 发送 `26/1 {type=2,id=value}`；`0x01033544`
  的 `7/7 type=2` 与 `7/11` 分支更新／删除当前背包行。
...
- 成功响应的背包同步复用已验证的 `7/7(type=2)+7/11`，不伪造商城或背包页面响应。
...
- `php scripts/dynamic-npc-equipment-buyer-regression.php`：隔离账号中六件相同
  ...
  `26/1 + 7/7 + 7/11`；重复提交同一序号不再加钱或下发删除更新。
- `make -j2`：通过。
