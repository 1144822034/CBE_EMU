...
客户端只会在元数据 id/code 与本地持久记录不一致时走上述更新分支。因此
WT 18/9 的 id 必须等于 WT 18/8 的 version，code 必须等于所有 18/8 data
分片的最终有符号字节累加和。
...
2. 将该资源纳入累积的内容失效清单，递增 release_id，并重新计算校验和。
...
id/code 对则被 `startup_screen_is_update_target_already_current()` 识别为已安装，既不
请求 18/8 也不删除资源。重复失效同一文件是协议允许的；缺失文件只会在下一次加载时
下载一次。
...
1. 18/9 响应的 type=1、id=77、code 为清单的有符号字节校验和。
