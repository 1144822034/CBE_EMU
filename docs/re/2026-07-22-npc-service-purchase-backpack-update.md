# NPC 服务商人购买后的背包更新（2026-07-22，2026-08-10 更正）

> `26/1 + 7/7` 是正确的购买数据契约，但不能让两个对象由同一个 `action=1`
> 回调消费。客户端远程传输层必须把它拆为先后两个普通网络事件；完整根因与
> WT 偏移证据见 `2026-08-10-dynamic-npc-purchase-progress-stall.md`。
...
动态 NPC 商人购买成功在线路上的响应顺序是：

```text
1/26/1 { dialog=购买成功后的菜单 }
1/7/7  { type=1, iteminfo=本次购买的一行 }
```

购买事务先写入关系型背包。远程客户端将精确的二对象组合分成两个 scheduler
网络事件：`26/1` 先结束 NPC 对话等待，随后 `7/7 type=1` 由
`TimerControl_ProcessItem` 作为一次增量插入/合并本次物品。`17/1` 仍只用于
背包页面自身的全量查询，不能代替该购买增量。

普通可堆叠物品的增量为 `1`。逍遥壶/神仙壶等储量型物品保持独立行，其 wire
count 使用新行的 HP/MP 储量；不能把它们错误降为普通物品的 `1`。
