...
> 2026-08-27 superseded：客户端不得按登录 WT 内容拆包或合成第二个 event-7。保留本记录
> 的 parser-pool 历史证据，但不再将宿主拆分视为兼容策略。2026-08-28 的当前服务端方案
> 不拆包，而是使用 CBE 已发送的 type-2/type-3 独立请求分阶段响应；详见
> [分阶段完整背包引导](2026-08-28-staged-full-backpack-bootstrap.md)。

## 当前临时兼容策略

2026-08-26，旧版 Android APK 登录后出现“解包错误”。原因是该 APK 不含客户端
分包逻辑，却收到包含完整四档强化词条的首登 `30/21` 组合包。

无活跃客户端会话的遗留直调和普通网格刷新仍对 `30/21` 装备行下发已持久化的**第一档**
`+4` 计划：`attrCount=1`，阈值为 `4`，类型和数值直接取装备实例的
`enhance_affix_types/values[0]`。普通物品仍是 `attrCount=0`。正常首次登录则不在 group
回复创建主实例，而在 CBE 随后的原生 type-2 请求中下发完整四档 `30/21`；`17/1` 和
`7/7 type=2` 仍保持完整四档详情表示。

该序列化不生成、修补或保存词条。旧数据在角色载入的
`vm_net_mock_role_normalize_*()` 阶段由
`vm_net_mock_equipment_enhancement_ensure_affixes()` 修补，并由关系型保存事务持久化；
响应阶段只读取已存在的第一档。
...
最低档方案每件装备只增加一个 typed 属性行（13 字节），而不是完整四档的 52 字节。
对已记录的 31 件装备大背包样本，完整行导致的 6283 字节解析分配，改为只携带首档后
按同一公式减少 `3 * 13 * 31 = 1209` 字节至 5074 字节；仍须通过真实客户端首登路径
验证它不触发“解包错误”。
...
- `src/server/mock_server_catalog.c`
  - `vm_net_mock_build_backpack_grid_iteminfo_blob()` 对装备使用
    `vm_net_mock_seq_put_item_first_stage_extra()`；它只读取 durable `+4` 词条。
...
- `scripts/first-login-equipment-attribute-bootstrap-regression.c`
  - 首登夹具新增一件 `+0` 背包装备，断言 `30/21` 行只有一条门槛为 `4`、与
    持久化实例一致的词条，而普通物品仍为零属性行。
  - 另构造历史最大形状的 48 格网格（31 件装备），逐行断言每件装备均只有其稳定的
    `+4` 词条；总 `iteminfo` 为 1699 字节。
