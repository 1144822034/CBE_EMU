...
2026-08-28 起，服务端对 `30/21` 中的装备行下发已持久化的**第一档** `+4` 计划：
`attrCount=1`，阈值为 `4`，类型和数值直接取装备实例的
`enhance_affix_types/values[0]`。普通物品仍是 `attrCount=0`。这使客户端首次创建
背包实例时至少拥有一条灰色强化词条；`17/1` 和 `7/7 type=2` 保持完整四档
`+4/+8/+12/+16` 的详情表示。
