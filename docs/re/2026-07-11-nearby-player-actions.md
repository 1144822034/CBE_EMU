...
（id、HP/MP、姓名、sexGroup、jobIndex），把自己的 id/HP/MP 编码为右侧行。
...
服务端的双人 duel 会话保存双方战斗 HP/MP、每轮双方 intent、提交掩码和带双端
投递掩码的整轮事件。首位 `4/2` 只保存 intent 并返回零对象 WT，不下发 `4/6`；
第二位提交后才按提交顺序结算两个 intent，再按上面的观察端映射分别生成同一个
双动作 `4/6 actioninfo`。duplicate 不覆盖首次 intent；自动 `4/12` 也复用同一
屏障。技能 MP 只在 duel 快照中扣除，并通过本机视角 `teaminfo` 刷新施法者缓存。
友好切磋的 HP/MP 不写回持久化角色状态，避免失败方在场景中留下 0 HP，或切磋
消耗永久 MP。

任一方 duel 临时 HP 到 0 时，最后一击仍由普通/技能 action 扣至 0，但不附带
普通怪物战斗的 type-3 死亡动作，避免失败方进入复活石与 `7/14` 链。双方都收到
最后一个 `4/6` 与无奖励的 `4/7` 结果对象同包下发，客户端播放动作后进入原生结算
面板；确认后才发送空 `25/5`。`4/8` 是复活专用，曾导致提前关闭和空白框，不能用于
自然切磋。服务端保留 finished duel 直到两端 `25/5` 都到达才释放。主动脱离仍使用
`4/4 {result=1}`，其固件
语义就是“逃跑成功”；断线端直接完成退出，仍在线对端走 `4/4 -> 25/5`。详细
根因与固件证据见
`docs/re/2026-08-19-duel-terminal-exit.md`。

2026-07-20 的双账号服务回归覆盖：拒绝、同意后双方 `4/10`、两方向普通攻击
镜像、旧版轮流行动、死亡动作、双方终止退出和主动脱离；stderr 为 0，未出现
未处理 WT。最终含技能与死亡回归的日志为
`tmp/mock-service-19090.20260720-003829.stdout.log`，对应 stderr 长度为 0。

2026-08-19 的 `duel-round-barrier-v1` 旧隔离回归覆盖 subtype 10 的
`sexGroup,jobIndex` 顺序、首位提交零对象确认、反向提交顺序、duplicate、双端
镜像双动作、原生 `4/11 -> 4/12`、致死回合屏障和原生终局退出。最终证据位于
`artifacts/automation/duel-round-barrier-v1-20260819T135411389Z-47320/`；回归直接
解析 `actioninfo`，断言两端 actor/target wire 互为镜像、终局无 type 3/4、
自然终局双端 `4/6 + 4/7 -> 25/5` 后才释放，并验证持久 HP/MP 未被 duel 临时值污染；
主动逃跑的 `4/4` 保持独立路径。
