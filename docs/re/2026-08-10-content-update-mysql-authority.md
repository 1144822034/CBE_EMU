...
已完成安装的 `version` 与 `codeVersion`。`startup_handle_update_target_metadata`
(`0x0103B59A`) 收到 `type=1` 后会进入更新/续传状态机；
`startup_screen_is_update_target_already_current()` 只服务于该状态机的临时续传判断，
不会把服务端的 `type=1` 自动变成“无需更新”。

因此服务端必须严格比较客户端上报的 `version/codeVersion` 与当前发布的 `id/code`：
完全相同则回 `type=0,id=0,code=0`，不请求 WT 18/8；任一不同才回 `type=1` 并让客户端
下载清单、逐项删除其中资源，随后在正常加载时通过 WT 18/7 重取需要的文件。服务端不按
启动次数递增版本，只有文件集合或已发布文件字节实际变化时才产生新版本。
